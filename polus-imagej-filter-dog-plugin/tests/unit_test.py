...
class UnitTest(unittest.TestCase):
    ...
    def test_plugin(self):
        ...
        _opName = op
        _in1_wipp_types = {'DoGSingleSigmas': 'collection', 'DoGVaryingSigmas': 'collection'}
        _in1_imagej_types =  {'DoGSingleSigmas': 'RandomAccessibleInterval', 'DoGVaryingSigmas': 'RandomAccessibleInterval'}
        if _in1_wipp_types.get(op, None) != 'collection':
            method_call_types.update({method:dtype for method,dtype in _in1_imagej_types.items() \
                if dtype in supported_data_types})
        _sigma1_wipp_types = {'DoGSingleSigmas': 'number'}
        _sigma1_imagej_types =  {'DoGSingleSigmas': 'double'}
        if _sigma1_wipp_types.get(op, None) != 'collection':
            method_call_types.update({method:dtype for method,dtype in _sigma1_imagej_types.items() \
                if dtype in supported_data_types})
        _sigma2_wipp_types = {'DoGSingleSigmas': 'number'}
        _sigma2_imagej_types =  {'DoGSingleSigmas': 'double'}
        if _sigma2_wipp_types.get(op, None) != 'collection':
            method_call_types.update({method:dtype for method,dtype in _sigma2_imagej_types.items() \
                if dtype in supported_data_types})
        _sigmas1_wipp_types = {'DoGVaryingSigmas': 'array'}
        _sigmas1_imagej_types =  {'DoGVaryingSigmas': 'double[]'}
        if _sigmas1_wipp_types.get(op, None) != 'collection':
            method_call_types.update({method:dtype for method,dtype in _sigmas1_imagej_types.items() \
                if dtype in supported_data_types})
        _sigmas2_wipp_types = {'DoGVaryingSigmas': 'array'}
        _sigmas2_imagej_types =  {'DoGVaryingSigmas': 'double[]'}
        if _sigmas2_wipp_types.get(op, None) != 'collection':
            method_call_types.update({method:dtype for method,dtype in _sigmas2_imagej_types.items() \
                if dtype in supported_data_types})
        # Generate data for the inputs
        _in1 = self.generate_data(
            'in1',
            _in1_wipp_types.get(op, None), 
            method_call_types.get(op, None)
            )
        _sigma1 = self.generate_data(
            'sigma1',
            _sigma1_wipp_types.get(op, None), 
            method_call_types.get(op, None)
            )
        _sigma2 = self.generate_data(
            'sigma2',
            _sigma2_wipp_types.get(op, None), 
            method_call_types.get(op, None)
            )
        _sigmas1 = self.generate_data(
            'sigmas1',
            _sigmas1_wipp_types.get(op, None), 
            method_call_types.get(op, None)
            )
        _sigmas2 = self.generate_data(
            'sigmas2',
            _sigmas2_wipp_types.get(op, None), 
            method_call_types.get(op, None)
            )
