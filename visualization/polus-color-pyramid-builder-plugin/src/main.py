...
def get_bounds(br, lower_bound, upper_bound):
    ...
    lower_bound_vals = np.full(
        (2 * lower_bound_size,), np.iinfo(dtype).max, dtype=dtype,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="main", description="Builds a DeepZoom color pyramid.",
    )
    ...
    parser.add_argument(
        ...
        dest="layout",
        type=str,
        help="Color ordering (e.g. 1,11,,,,5,6)",
        required=True,
    )
    parser.add_argument(
        "--bounds",
        dest="bounds",
        type=str,
        help="Set bounds (should be float-float, int-int, or blank, e.g. 0.01-0.99,0-16000,,,,,)",
        required=False,
    )
    parser.add_argument(
        "--alpha",
        dest="alpha",
        type=str,
        help="If true, transparency is equal to pixel intensity in the pyramid.",
        required=False,
    )
    parser.add_argument(
        "--stitchPath",
        dest="stitch_path",
        type=str,
        help="Path to a stitching vector.",
        required=False,
    )
    parser.add_argument(
        "--background",
        dest="background",
        type=str,
        help="Background fill value.",
        required=False,
    )

    # Output arguments
    parser.add_argument(
        "--outDir", dest="outDir", type=str, help="Output pyramid path.", required=True,
    )
    ...
    if "c" not in fp.variables:
        msg = "A channel variable is expected in the filepattern."
    ...
    for files in fp.iterate(group_by=fp.variables):
        count += 1
        outDirFrame = outDir.joinpath(f"{count}_files")
        outDirFrame.mkdir()
        bioreaders = []
        threads = []
        with ThreadPoolExecutor(max([multiprocessing.cpu_count() // 2, 2])) as executor:
            for i, l in enumerate(layout):
                ...
                if stitch_path is None:
                    ...
                    bioreaders.append(
                        BioReader(f_path, max_workers=multiprocessing.cpu_count()),
                    )
                else:
                    ...
                    for stitch in Path(stitch_path).iterdir():
                        br = utils.BioAssembler(
                            inpDir, stitch, multiprocessing.cpu_count(),
                        )
                ...
                if layout[i] is not None:
                    if isinstance(bounds[i][0], float):
                        logger.info(
                            "{}: Getting percentile bounds {}...".format(
                                Path(bioreaders[-1]._file_path).name, bounds[i],
                            ),
                        )
                        threads.append(
                            executor.submit(
            ...
            for i in reversed(range(len(layout))):
                if bioreaders[i] is None:
                    continue
                if isinstance(bounds[i][0], int) or bounds[i][0] is None:
                    logger.info(
                        "Color {}: {} (rescaling to {})".format(
                            COLORS[i],
