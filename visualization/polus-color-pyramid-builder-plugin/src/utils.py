...
class BioAssembler:
    def __init__(self, file_path, stitch_path, max_workers=None) -> None:
    ...
    def buffer_image(self, image_path, Xi, Yi, Xt, Yt, color=False):
        ...
        if color is not None:
            image_temp = (
                255
                * (image[..., 0, 0].astype(np.float32) - self.bounds[0])
    ...
    def make_tile(self, x_min, x_max, y_min, y_max, color=None):
        ...
        # Get the data type
        br = BioReader(
            str(Path(self._file_path).joinpath(self._file_dict["filePos"][0]["file"])),
        )
        dtype = br._pix["type"]
        ...
        with ThreadPoolExecutor(max([self._max_workers, 2])) as executor:
            for f in self._file_dict["filePos"]:
                if ((f["posX"] >= x_min and f["posX"] <= x_max) or (
                    f["posX"] + f["width"] >= x_min and f["posX"] + f["width"] <= x_max
                )) and ((f["posY"] >= y_min and f["posY"] <= y_max) or (
                    ...
                    executor.submit(
                        self.buffer_image,
                        str(Path(self._file_path).joinpath(f["file"])),
                        Xi,
                        Yi,
                        Xt,
                        Yt,
        ...
        with open(fpath) as fr:
            ...
            for line in fr:
                ...
                stitch_groups["width"], stitch_groups["height"] = BioReader.image_size(
                    str(Path(imagePath).joinpath(stitch_groups["file"]).absolute()),
                )
        ...
        if X[0] >= self.X[0] and X[1] <= self.X[1]:
            if Y[0] >= self.Y[0] and Y[1] <= self.Y[1]:
                if Z[0] >= self.Z[0] and Z[1] <= self.Z[1]:
                    return self._image[
                        Y[0] - self._Y_offset : Y[1] - self._Y_offset,
...
def _get_higher_res(
    ...
    channels = [[0, 3], [1, 3], [2, 3], [0, 1, 3], [0, 2, 3], [1, 2, 3], [0, 1, 2, 3]]
    ...
    if str(S) == encoder.info["scales"][0]["key"]:
        for ind, br in enumerate(bfio_reader):
            ...
            if isinstance(br, BioAssembler):
                ...
                image_temp = (
                    255
                    * (
                        br.read_image(X=X, Y=Y, Z=Z)[..., 0, 0].astype(np.float32)
                        - br.bounds[0]
                    )
                    / (br.bounds[1] - br.bounds[0])
                )
                image_temp[image_temp > 255] = 255
                image_temp[image_temp < 0] = 0
                image_temp = image_temp.astype(np.uint8)
        ...
        if (
            S % 2 == 0 or str(S + 1) == encoder.info["scales"][0]["key"]
        ) and not stitch:
...
class PyramidWriter:
    ...
    def store_chunk(self, buf, key, chunk_coords):
        ...
        try:
            ...
            msg = f"Error storing chunk {self._chunk_path(key, chunk_coords)} in {self.base_path}: {exc}"
...
class NeuroglancerChunkEncoder:
    ...
    def __init__(self, info) -> None:
        ...
        if data_type not in self.DATA_TYPES:
            msg = f"Invalid data_type {data_type} (should be one of {self.DATA_TYPES})"
            raise KeyError(
                msg,
            )
...
def bfio_metadata_to_slide_info(bfio_reader, outPath):
    ...
    resolution.append(
        (phys_y[0] * UNITS[phys_y[1]] + phys_x[0] * UNITS[phys_x[1]]) / 2,
...
def dzi_file(bfio_reader, outPath, imageNum):
    ...
    op = Path(outPath).parent.joinpath(f"{imageNum}.dzi")
    ...
    with open(op, "w") as writer:
        writer.write(
            DZI.format(
                CHUNK_SIZE, info["scales"][0]["size"][0], info["scales"][0]["size"][1],
            ),
        )
