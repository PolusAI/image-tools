...
def median(data_list):
    ...
    data_list.sort()
    return (data_list[(num_obj - 1) // 2] + data_list[num_obj // 2]) / 2
...
def skewness(data_list):
    ...
    return sum([(x - mu) ** 3 for x in data_list]) / (n * sigma**3)
...
def kurtosis(data_list):
    ...
    return sum([(x - mu) ** 4 for x in data_list]) / (n * sigma**4) - 3
...
def _parse_stitch(stitchPath, fp):
    ...
    txt_files = [
        f.name for f in Path(stitchPath).iterdir() if f.is_file() and f.suffix == ".txt"
    ]
    global_regex = ".*-global-positions-([0-9]+).txt"
    stitch_vectors = [
        re.match(global_regex, f)
        for f in txt_files
        if re.match(global_regex, f) is not None
    ]

    line_regex = (
        r"file: (.*); corr: (.*); position: \((.*), (.*)\); grid: \((.*), (.*)\);"
    )
    ...
    for vector in stitch_vectors:
        ...
        with open(fpath) as fr:
            ...
            for line in fr:
                ...
                stitch_groups = dict(zip(STITCH_VARS, stitch_groups.groups()))
                ...
                current_image.update(
                    {key: val for key, val in stitch_groups.items() if key != "file"},
                )
                ...
                # Get the image size
                current_image["width"], current_image["height"] = BioReader.image_size(
                    current_image["file"],
                )
                ...
                if fnum // 1000 > fcheck:
                    fcheck += 1
                    logger.info(f"Files parsed: {fnum}")
...
def _parse_features(featurePath, fp, method):
    ...
    csv_files = [
        f.name
        for f in Path(featurePath).iterdir()
    ...
    for feat_file in csv_files:
        ...
        with open(fpath) as fr:
            ...
            var_ind = dict(enumerate(headers))  # map headers to line positions
            ...
            feature_list.update(
                {
                    key: []
                    for key in headers
                    if key not in feature_list.keys() and key != "file"
                },
            )
            ...
            while line:
                ...
                p_line = {
                    var_ind[ind]: val
                    for ind, val in enumerate(line.rstrip("\n").split(","))
                }
                ...
                if current_image is None or "line" not in current_image.keys():
                    line = fr.readline()
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="features",
        type=str,
        help="CSV collection containing features",
        required=True,
    )
    parser.add_argument(
        "--inpDir",
        dest="inpDir",
        type=str,
        help="Input image collection used to build a pyramid that this plugin will make an overlay for",
        required=True,
    )
    parser.add_argument(
        "--method",
        dest="method",
        type=str,
        help="Method used to create the heatmap",
        required=True,
    )
    parser.add_argument(
        "--vector",
        dest="vector",
        type=str,
        help="Stitching vector used to buld the image pyramid.",
        required=True,
    )
    parser.add_argument(
        "--outImages",
        dest="outImages",
        type=str,
        help="Heatmap Output Images",
        required=True,
    )
    parser.add_argument(
        "--vectorInMetadata",
        dest="vectorInMetadata",
        type=str,
        help="Store stitching vector in metadata",
        required=True,
    )
    parser.add_argument(
        "--outVectors",
    ...
    logger.info(f"vectorInMetadata = {vectorInMetadata}")
    if vectorInMetadata:
    ...
    logger.info(f"outImages = {outImages}")
    logger.info(f"outVectors = {outVectors}")
    ...
    for key, val in feature_list.items():
        valid_vals = [v for v in val if v != "NaN"]
        if len(valid_vals) == 0:
            feature_mins[key] = 0
    ...
    for fl in fp.iterate():
        ...
        for ft in feature_list:
            try:
                if get_number(fl[ft]):
                    fl[ft] = round(
                        (fl[ft] - feature_mins[ft]) / feature_ranges[ft] * 254 + 1,
                    )
    ...
    jutil.start_vm(
        args=[f"-Dlog4j.configuration=file:{log_config.absolute()!s}"],
    ...
    for w in widths:
        for h in heights:
            for l in unique_levels:
                out_file = Path(outImages).joinpath(
                    str(w) + "_" + str(h) + "_" + str(l) + ".ome.tif",
                )
                if not out_file.exists():
                    ...
                    bw = BioWriter(
                        str(
                            Path(outImages)
                            .joinpath(str(w) + "_" + str(h) + "_" + str(l) + ".ome.tif")
                            .absolute(),
                        ),
                        X=w,
                        Y=h,
    ...
    for num, feat in enumerate(feature_list):
        fpath = str(
            Path(outVectors)
            .joinpath("img-global-positions-" + str(num + 1) + ".txt")
            .absolute(),
        )
                ...
                if "line" in f and f["line"] == line:
                    fw.write(
                        "file: {}; corr: {}; position: ({}, {}); grid: ({}, {});\n".format(
                            file_name.format(f["width"], f["height"], f[feat]),
                            f["correlation"],
                            f["posX"],
                            f["posY"],
                            f["gridX"],
