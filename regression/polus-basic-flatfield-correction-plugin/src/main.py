import argparse, logging, typing, pathlib
...
def main(
    ...
    if file_pattern is None:
        ...
        _, group_by = get_regex(file_pattern)
        for v in "ztrc":
            group_by = group_by.replace(v, "")

    fp = FilePattern(input_dir, file_pattern)

    ProcessManager.num_processes(max([1, cpu_count() // 4]))
    ProcessManager.threads_per_process(4)
    ProcessManager.init_processes("basic")

    file_list = [[f[0] for f in fp()]] if group_by is None else fp(group_by=group_by)
