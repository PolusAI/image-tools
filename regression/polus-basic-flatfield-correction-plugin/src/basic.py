...
import numpy as np
from filepattern import FilePattern, infer_pattern
...
def _get_resized_image_stack(flist):
    ...
    When files are parsed, the variables are used in an index to provide a method to
    reference a specific file name by its dimensions. This function returns the variable
    index based on the input filename pattern.

    To help reduce memory overhead, the number of images loaded for fitting are limited
    to OPTIONS['n_sample'] number of images. If there are more than n_sample number of
    images, images are selected at random.
    ...
    def load_and_store(fname, ind):
        with ProcessManager.thread():
            with BioReader(fname["file"]) as br:
...
def _dct2(imgflt_stack):
    ...
    This function originally existed to replicate Matlabs dct function using scipy's dct
    function. It was necessary to perform the dct along the rows and columns. To
    simplify the container creation, the dct function from opencv was used. This
    function is slower, and scipy's version should be implemented in the future.
...
def _idct2(imgflt_stack):
    ...
    The scipy dct that was originally used was type-III, which is the inverse of dct
    type-II
...
def _initialize_options(img_stack, get_darkfield, options):
    ...
    This function modifies the default OPTIONS using information about the images to be
    processed.
...
def _inexact_alm_l1(imgflt_stack, options):
    ...
    This function finds the smallest number of features in Fourier space (frequencies)
    that minimizes the error when compared to images, using L1 loss and IALM
    optimization. By using L1 loss in Fourier space, the resulting background flatfield
    image should be an image with smooth gradients.
    ...
    A_coeff = np.ones(
        (1, img_3d), dtype=np.float64
    )  # per image scaling coefficient, accounts for things like photobleaching
    A_offset = np.zeros(
        (img_size, 1), dtype=np.float64
    )  # offset per pixel across all images
    ...
    while not converged:
        ...
        weight_upd = np.max(
            np.reshape(
                weight_upd - options["lambda"] / (ent1 * mu), (img_width, img_height, 1)
            ),
            -1,
            initial=0,
        ) + np.min(
        ...
        E1 = np.max(
            np.reshape(E1 - options["weight"] / (ent1 * mu), (img_size, img_3d, 1)),
            -1,
            initial=0,
        ) + np.min(
            np.reshape(E1 + options["weight"] / (ent1 * mu), (img_size, img_3d, 1)),
        ...
        if options["darkfield"]:
            ...
            R1_upper = R1[
                np.argwhere(
                    np.reshape(W_idct_hat, (-1, 1)).astype(np.float64)
                    > (np.float64(np.mean(W_idct_hat)) - np.float64(10 ** -5))
                )[:, 0],
                :,
            ]
            ...
            R1_lower = R1[
                np.argwhere(
                    np.reshape(W_idct_hat, (-1, 1))
                    < np.mean(W_idct_hat) + np.float64(10 ** -5)
                )[:, 0],
                :,
            ]
            ...
            B1_offset = np.min(
                B1_offset, initial=B1_uplimit / (np.mean(W_idct_hat) + 10 ** -7)
            )
            B_offset = B1_offset * np.mean(W_idct_hat) - B1_offset * np.reshape(
                W_idct_hat, (-1, 1)
            )

            # Calculate darkfield
            A1_offset = np.reshape(
                np.mean(R1[:, validA1coeff_idx], 1), (-1, 1)
            ) - np.mean(A_coeff[0, validA1coeff_idx]) * np.reshape(W_idct_hat, (-1, 1))
            ...
            W_offset = np.max(
                np.reshape(
                    W_offset - options["lambda_darkfield"] / (ent2 * mu),
            ...
            A_offset = np.max(
                np.reshape(
                    A_offset - options["lambda_darkfield"] / (ent2 * mu),
                    (A_offset.shape[0], A_offset.shape[1], 1),
                ),
                -1,
                initial=0,
            ) + np.min(
        ...
        if (
            stopCriterion < options["optimization_tol"]
            or iternum > options["max_iterations"]
        ):
            converged = True
...
def _get_flatfield_and_reweight(flt_pxl, pxl_err, df_pxl, options):
    ...
    The inexact augmented legrangian multiplier method uses L1 loss, but this is only
    done since an exact solution to L0 problems do not exist. After each round of
    optimization, the starting weights are recalculated to give low weights to pixels
    that are not background and high weights to background pixels. Then another round of
    optimization is performed.

    Since the value of the weights are tied to the values of the flatfield and darkfield
    values, this function formats both the flatfield and darkfield images for output and
    returns an updated weight matrix.

    The flatfield image is normalized so that the mean pixel value is 1. The darkfield
    image contains raw pixel values.
    ...
    Outputs:
        flatfield - A floating precision numpy matrix containing normalized flatfield
            values
        darkfield - A floating precision numpy matrix containing darkfield pixel values
        options - optimization options with new weights (this is passed only for code
            readability)
    """
    XA = np.reshape(flt_pxl, (options["size"], options["size"], -1))
    XE = np.reshape(pxl_err, (options["size"], options["size"], -1))
    XE_norm = XE / np.tile(
        np.reshape(np.mean(np.mean(XA, 0), 0) + 10 ** -6, (1, 1, -1)),
...
def basic(
    ...
    with ProcessManager.process(base_output):
        ...
        flatfield_old = np.ones(
            (new_options["size"], new_options["size"]), dtype=np.float64
        )
        darkfield_old = np.random.normal(
            size=(new_options["size"], new_options["size"])
        ).astype(np.float64)
        ...
        for w in range(new_options["max_reweight_iterations"]):
            ...
            flatfield, darkfield, new_options = _get_flatfield_and_reweight(
                A, E1, A_offset, new_options
            )

            # Calculate the change in flatfield and darkfield images between iterations
            mad_flat = np.sum(np.abs(flatfield - flatfield_old)) / np.sum(
                np.abs(flatfield_old)
            )
            temp_diff = np.sum(np.abs(darkfield - darkfield_old))
            if temp_diff < 10 ** -7:
                mad_dark = 0
            else:
                mad_dark = temp_diff / np.max(
                    np.sum(np.abs(darkfield_old)), initial=10 ** -6
                )
        ...
        flatfield = cv2.resize(flatfield, (Y, X), interpolation=cv2.INTER_CUBIC).astype(
            np.float32
        )
        if new_options["darkfield"]:
            darkfield = cv2.resize(
                darkfield, (Y, X), interpolation=cv2.INTER_CUBIC
            ).astype(np.float32)
