#from uutility import ConvertImage, Df_Csv, Analysis
...
def list_file(img_directory):
    ...
    list_of_files: string
        Contains the path to directory, list of names of the subdirectories in dirpath (if any) and the filenames of .ome.tif files
    """
...
def read(img_file):
    ...
    Returns
    -------
    image_squeeze: (N,M) ndarray
        Array of the image.
    img_unit: string
        The embedded unit in the metadata if present else it will be none.
    """
...
def box_border_search(label_image, boxsize=3):
    ...
    -------
    Perimeter : array
        A array containing the perimeter pixels of the object n.
    """
...
def neighbors_find(lbl_img,pixeldistance):
    ...
    Args
    ----
    lbl_image : (N,M) ndarray
        Labeled image array.
    boxsize : int
        Box size value.
    pixeldistance : int
        Pixel distance value.
    ...
    Notes
    -----
    Number_of_Neighbors = neighbors(label_image,boxsize,pixeldistance = None)
...
def feret_diameter(lbl_img,thetastart,thetastop):
    ...
    boxsize : int
        Box size value.
    thetastart : int
        Angle start value by default it is 1.
    thetastart : int
        Angle stop value by default it is 180.
...
def polygonality_hexagonality(area, perimeter, neighbors, solidity, maxferet, minferet):
    ...
    maxferet : float
        Maximum caliper distance across the entire object.
    minferet : float
        Minimum caliper distance across the entire object.
...
def feature_extraction(label_image,features,seg_file_names1,embeddedpixelsize, img_emb_uint, unitLength, pixelsPerunit,intensity_image,pixelDistance=5):
    ...
    seg_file_names1 : string
        Filename of the labeled image.
    embeddedpixelsize : boolean
        If true will consider units from metadata.
    img_emb_unit : string
        Units embedded in the metadata.
    unitLength : string
        Required units for the extracted features.
    pixelsPerunit : int
        Pixels per unit for the metric mentioned in unitLength.
    intensity_image : (N,M) ndarray
        Intensity image array.
    pixelDistance : int
        Distance between pixels to calculate the neighbors touching the object and default valus is 5.
    
    Returns
    -------
    df : Dataframe.
        Dataframe containing the features extracted.
    title : string.
        Filename of the labeled image.
    """ 
    ...
    def neighbors(seg_img,*args):
        ...
        neighbor_array = neighbors_find(edges, pixeldistance=5)
    ...
    def maxferet(seg_img,*args):
        ...
        edges= box_border_search(seg_img, boxsize)
        feretdiam = feret_diameter(edges,thetastart,thetastop)
    ...
    for each_feature in features:
       ...
       if each_feature  == 'all':
           ...
           if embeddedpixelsize:
               df.columns =['Area-%s'%img_emb_uint,'Centroid row','Centroid column','Convex area-%s'%img_emb_uint,'Eccentricity','Equivalent diameter-%s'%img_emb_uint,'Euler number','Hexagonality score','Hexagonality sd','Kurtosis','Major axis length-%s'%img_emb_uint,'Maxferet-%s'%img_emb_uint,'Maximum intensity','Mean intensity','Median','Minimum intensity','Minferet-%s'%img_emb_uint,'Minor axis length-%s'%img_emb_uint,'Mode','Neighbors','Orientation','Perimeter-%s'%img_emb_uint,'Polygonality score','Standard deviation','Skewness','Solidity']
           #if unitlength metric is considered
           elif unitLength and not embeddedpixelsize:
               df.columns =['Area-%s^2'%unitLength,'Centroid row','Centroid column','Convex area-%s^2'%unitLength,'Eccentricity','Equivalent diameter-%s'%unitLength,'Euler number','Hexagonality score','Hexagonality sd','Kurtosis','Major axis length-%s'%unitLength,'Maxferet-%s'%unitLength,'Maximum intensity','Mean intensity','Median','Minimum intensity','Minferet-%s'%unitLength,'Minor axis length-%s'%unitLength,'Mode','Neighbors','Orientation','Perimeter-%s'%unitLength,'Polygonality score','Standard deviation','Skewness','Solidity']
           #units in pixels
           else:
           ...
           if 'Area' or 'Convex area' or 'Equivalent diameter' or 'Major axis length' or 'Maxferet' or 'Minor axis length' or 'Minferet' or 'Perimeter' in df.columns:
               if embeddedpixelsize:   
                   df.rename({"area": "Area-%s"%img_emb_uint , "convex_area": "Convex area-%s"%img_emb_uint, "equivalent_diameter": "Equivalent diameter-%s"%img_emb_uint, "major_axis_length": "Major axis length-%s"%img_emb_uint, "minor_axis_length": "Minor axis length-%s"%img_emb_uint,"maxferet": "Maxferet-%s"%img_emb_uint, "minferet": "Minferet-%s"%img_emb_uint,"perimeter": "Perimeter-%s"%img_emb_uint},axis='columns',inplace =True)
               elif unitLength and not embeddedpixelsize:
                   df.rename({"area": "Area-%s^2"%unitLength , "convex_area": "Convex area-%s^2"%unitLength, "equivalent_diameter": "Equivalent diameter-%s"%unitLength, "major_axis_length": "Major axis length-%s"%unitLength, "minor_axis_length": "Minor axis length-%s"%unitLength,"maxferet": "Maxferet-%s"%unitLength, "minferet": "Minferet-%s"%unitLength,"perimeter": "Perimeter-%s"%unitLength},axis='columns',inplace =True)
               else:
...
def main():
    ...
    try:
        ...
        for segfile in configfiles_seg:
            ...
            df,title = feature_extraction(label_image,features,seg_file_names1,embeddedpixelsize, img_emb_uint, unitLength, pixelsPerunit,intensity_image, pixelDistance)
