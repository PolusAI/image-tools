...
def write_thread(
    out_file_path: Path, data: np.ndarray, metadata: OmeXml, chan_name: str,
):
...
def extract_fovs(file_path: Path, out_path: Path):
    ...
    with ProcessManager.process(file_path.name):
        ...
        row_conv = dict(zip(
                np.unique(np.sort(ind["Y"])), range(0, len(np.unique(ind["Y"]))),
            ))
        col_conv = dict(zip(
                np.unique(np.sort(ind["X"])), range(0, len(np.unique(ind["X"]))),
            ))
        ...
        for s, i in zip(subblocks, range(0, len(subblocks))):
            ...
            out_file_path = out_path.joinpath(
                _get_image_name(
                    base_name, row=ind["Row"][i], col=ind["Col"][i], Z=Z, C=C, T=T,
                ),
            )
