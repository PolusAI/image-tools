...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="channelOrder",
        type=str,
        help="Order in which to place the channels (e.g. 1,3,2)",
        required=True,
    )
    ...
    logger.info(f"inpDir = {Path(inpDir).resolve()}")
    channelOrder = [int(c) for c in str(args.channelOrder).split(",")]
    logger.info(f"channelOrder = {channelOrder}")
    outDir = args.outDir
    ...
    try:
        ...
        for files in fp.iterate(group_by="c"):
            ...
            file_name = filepattern.output_name(
                filePattern, paths, {c: paths[0][c] for c in fp.variables if c != "c"},
            )
            logger.info(f"Writing: {file_name}")
            bw = BioWriter(
                str(Path(outDir).joinpath(file_name)), metadata=br.read_metadata(),
            )
            del br
            ...
            total = (
                bw.num_c()
                * bw.num_z()
                * (bw.num_x() // chunk_size + 1)
                * (bw.num_y() // chunk_size + 1)
            )
                    ...
                    for z in range(br.num_z()):
                        ...
                        for x in range(0, br.num_x(), chunk_size):
                            ...
                            for y in range(0, br.num_y(), chunk_size):
                                ...
                                threads.append(
                                    executor.submit(write_tile, br, bw, X, Y, Z, C),
                                )
