...
ThreadFuture = Tuple[
    Optional[numpy.ndarray], numpy.ndarray, numpy.ndarray, numpy.uint32
]
    ...
    if previous_labels.size != 0 and current_labels.size != 0:
        ...
        for label in current_labels:
            ...
            new_labels, counts = numpy.unique(
                previous_values[current_values == label], return_counts=True
            )
...
def vector_thread(
    *,
    ...
    with ProcessManager.thread():
        ...
        x_min, x_max = max(0, x - utils.TILE_OVERLAP), min(
            x_shape, x + utils.TILE_SIZE + utils.TILE_OVERLAP
        )
        y_min, y_max = max(0, y - utils.TILE_OVERLAP), min(
            y_shape, y + utils.TILE_SIZE + utils.TILE_OVERLAP
        )
        z_min, z_max = max(0, z - utils.TILE_OVERLAP), min(
            z_shape, z + utils.TILE_SIZE + utils.TILE_OVERLAP
        )
        ...
        with BioReader(in_path) as reader:
            ...
            mask = numpy.squeeze(
                reader[y_min:y_max, x_min:x_max, z_min:z_max, 0, 0] > 0
            )[..., numpy.newaxis]
        ...
        labels = dynamics.vectors_to_labels(flows, mask, 1)
        ...
        if y > 0:
            labels, labels_y, indices_y = reconcile_overlap(
                future_y.squeeze(), current_y, labels
            )
        if x > 0:
            labels, labels_x, indices_x = reconcile_overlap(
                future_x.squeeze(), current_x, labels
            )
        if z > 0:
            labels, labels_z, indices_z = reconcile_overlap(
                future_z.squeeze(), current_z, labels
            )
...
def zarr_to_tif(zarr_path: Path, out_path: Path):
    with BioReader(zarr_path, max_workers=utils.NUM_THREADS) as reader:
        with BioWriter(
            out_path, metadata=reader.metadata, max_workers=utils.NUM_THREADS
        ) as writer:
            writer.dtype = numpy.uint32
...
def vector_to_label(
    *,
    ...
    with ProcessManager.process(in_path.name):
        ...
        for z_index, z in enumerate(range(0, reader_shape[0], utils.TILE_SIZE)):
            for y_index, y in enumerate(range(0, reader_shape[1], utils.TILE_SIZE)):
                for x_index, x in enumerate(range(0, reader_shape[2], utils.TILE_SIZE)):
                    ...
                    ProcessManager.submit_thread(vector_thread, **thread_kwargs)
                    threads[(z_index, y_index, x_index)] = ProcessManager._threads[-1]
...
def main(
    *,
    ...
    files = list(filter(lambda file_path: str(file_path).endswith(".ome.zarr"), files))

    ProcessManager.num_processes(utils.NUM_THREADS)
    ProcessManager.init_processes("vector-to-label")
    ...
    for in_path in files:
        ...
        ProcessManager.submit_process(
            vector_to_label, in_path=in_path, output_dir=output_dir
        )
