...
def _shift_and_scale(
    ...
    angles = numpy.multiply(angles, masks)
...
def flow_angles(
    ...
    """ Takes an array of flows with y-x components and produces arrays of masks
     and flow-angles.
    ...
    # For cosines
    thetas_cos = flows[1, ...] / numpy.maximum(flow_magnitudes, flow_magnitude_threshold)
    ...
    # For sines
    thetas_sin = flows[0, ...] / numpy.maximum(flow_magnitudes, flow_magnitude_threshold)
...
def find_boundary(masks: numpy.ndarray, angles: numpy.ndarray) -> numpy.ndarray:
    ...
    differences: List[numpy.ndarray] = [
    ...
    differences: Iterable = (
        ...
        for difference in map(numpy.abs, differences)
    )
    ...
    binary_boundaries = numpy.logical_and(masks, binary_boundaries)
...
def boundary_to_cells(masks: numpy.ndarray, boundary: numpy.ndarray) -> numpy.ndarray:
    ...
    cells = numpy.asarray(cells != 0, dtype=bool)
    return cells
...
def flows_to_labels(
    ...
    Args:
        flows: float array of shape (2, Y, X) containing the y and x components of flow-vectors.
        flow_magnitude_threshold: The minimum flow magnitude at a pixel for it to be considered part of a cell.
    ...
    # TODO: Should this also be done in a binary-ops plugin? The thought being that small objects should be removed
    #  before this step of growing the size of each object.
    # footprint = numpy.asarray([[0, 1, 0], [1, 1, 1], [0, 1, 0]], dtype=cells.dtype)
    for _ in range(1):
