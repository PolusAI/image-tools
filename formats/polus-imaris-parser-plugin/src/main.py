...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="inpdir",
        type=str,
        help="Input collection of ims files",
        required=True,
    )
    ...
    parser.add_argument(
        "--outdir", dest="outdir", type=str, help="Output csv collection", required=True,
    )
    ...
    inpdir = args.inpdir
    logger.debug(f"Old input directory: {inpdir}")
    ...
    try:
        ...
        # If given input directory points to parent of /metadata_files
        elif parent_meta_path.exists() is True:
            logger.info("Switching to subdirectory metadata_files")
            ...
            logger.error(
                "Directory not found. Please check that the input \
            directory is an image collection with at least one .ims file",
            )
            msg = "metadata_files directory not found."
            raise FileNotFoundError(msg)

        logger.info("Navigated to metadata_files directory...")
        logger.debug(f"New input directory (inpdir): {inpdir}")
        ...
        logger.debug(f"metaoutdir = {metaoutdir}")
        ...
        for currentFile in currentDirectory.iterdir():
            if currentFile.suffix == ".ims":
                ims_exists = True
                logger.info(f"Parsing {currentFile}")
