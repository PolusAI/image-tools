...
class CreateCsv:
    ...
    def __init__(self, ims_filename, dir_name, meta_dir_name, logger=None) -> None:
    ...
    def get_df_from_csv(self, dirname, chan, chan_name, csv_substring):
        ...
        if len(df.columns) > 0:
            ...
            df.columns = [
                "{}{}".format(c, "" if c in keepsame else chan_name) for c in df.columns
            ]
    ...
    def get_overall(self, overall_df, chan_name):
        ...
        overall_df["ID_Object" + chan_name] = overall_df[
            "ID_Object" + chan_name
        ].replace(-1.0, np.NaN, inplace=True)
        ...
        overall_df = pd.melt(
            overall_df,
            id_vars=["ID_Time", "ID_Object" + chan_name],
            var_name="Variable",
            value_name="Value",
        )
        ...
        return overall_df.dropna(axis=1, how="all")
    ...
    def create_overall_xlsx(self, imaris_filename, meta_dirname, all_overall_dict):
        ...
        for chan_name, overall_df_list in all_overall_dict.items():
            for i in range(0, len(overall_df_list)):
                ...
                overall_df_list[i]["Value"] = overall_df_list[i]["Value"].apply(
                    self.round_to_six,
                )
    ...
    def create_final_output(self, imaris_filename, non_overall_dfs, dirname):
        ...
        for chan_name, non_ov in non_overall_dfs.items():
            ...
            for i in range(0, len(non_ov)):
                ...
                non_ov[i].columns = non_ov[i].columns.str.replace("ID_Time", "Time")
                non_ov[i].columns = non_ov[i].columns.str.replace("ID_Object", "ID")
    ...
    def create_csv_fun(self):
        ...
        self.logger.debug(f"Opening .ims file {self.ims_filename!s}...")
        self.f = h5py.File(self.ims_filename, "r")
        ...
        channel_names = [
            ...
            if chan.startswith("Points") or chan.startswith("MegaSurfaces")
        ]
        ...
        for i in range(0, len(channel_names)):
            ...
            self.logger.debug(f"Reading {current_channel}...")
            ...
            #: Read the required input files
                ...
                if path.exists(path1) is True and path.exists(path2) is True:
                    #: Load Track Data
                    ...
                    if object_df.empty is True:
                        has_object = False

                    if track_id_object_df.empty is True:
                        track_id_object_df = pd.DataFrame(
                            {
                                "TrackID" + chan_name: np.NaN,
                                "ID_Object" + chan_name: np.NaN,
                            },
                            index=[0],
                        )
                        has_track_id_object is True

                    #: Isolate "Overall" data
                    if (has_track_id_object is True and has_object is True) or (
                        has_track_id_object is True and has_object is False
                    ):
                        #: Add 1 to all time chans (sets t=0 to t=1)
                        object_df["ID_Time"] = object_df["ID_Time"] + 1

                        #: Where Object ID < 0, save as "Overall"
                        overall_df = object_df.loc[
                    ...
                    if has_object is True and has_track is False:
                        ...
                        if excel_channel in non_overall_dfs:
                            non_overall_dfs[excel_channel].append(track_id_object_df)

                        else:
                            non_overall_dfs[excel_channel] = []
                            non_overall_dfs[excel_channel].append(track_id_object_df)

                    elif has_object is False and has_track is True:
                        track_id_object_df = pd.merge(
                            track_id_object_df,
                            track_df,
                            how="outer",
                    ...
                    #: Fix issue overwrite for files sharing chan name
                    elif has_object is True and has_track is True:
                        ...
                        if excel_channel in non_overall_dfs:
                            non_overall_dfs[excel_channel].append(features_merged)
        ...
        self.logger.info(f"{self.ims_filename!s} complete!")
