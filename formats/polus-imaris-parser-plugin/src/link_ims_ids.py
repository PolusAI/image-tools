...
def link_trackid_objectid(ims_filename, dir_name):
    ...
    for ims_channel in channel_names:
        #: Create Track0 df
        contains_trackobject0 = (f["Scene8"]["Content"][ims_channel]).__contains__(
            "TrackObject0",
        )

        contains_track0 = (f["Scene8"]["Content"][ims_channel]).__contains__("Track0")

        if contains_trackobject0 is True and contains_track0 is True:
            ...
            track_object_ids = pd.merge_asof(
                trackobject0.reset_index(),
                track0_df,
                left_on="index",
                right_on="IndexTrackObjectBegin",
            ).reindex(["ID_Object", "ID"], axis=1)
