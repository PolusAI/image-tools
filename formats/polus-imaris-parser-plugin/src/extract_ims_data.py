...
class LinkData:
    ...
    def __init__(self, ims_filename, dir_name, logger=None) -> None:
    ...
    def get_factor(self, h5py_file, chan_name):
        ...
        if df_factor.empty:
            df_factor = pd.DataFrame(
                [[0, "Overall", "Overall"]], columns=["ID_List", "Name", "Level"],
            )
    ...
    def get_statisticstype(self, h5py_file, chan):
        ...
        df_statistics_type = pd.DataFrame.from_records(
            stat_types, columns=["ID", "ID_Category", "ID_FactorList", "Name", "Unit"],
        )
        ...
        df_category.rename(
            columns={"ID": "catID", "Name": "redundant_cat_name"}, inplace=True,
        )
    ...
    def convert_byte_to_string_and_format(self, f_st):
        ...
        f_st[str.encode("Channel")] = f_st[str.encode("Channel")].str.decode("utf-8")
        ...
        f_st["Name"] = (
            f_st["Name"]
            + f_st["Unit"]
            + f_st[str.encode("Image")]
            + f_st[str.encode("Channel")]
        )
    ...
    def create_object_csv(self, df, chan):
        ...
        object_df.drop(
            columns=["ID_StatisticsType", "ID_FactorList"], axis=1, inplace=True,
        )
    ...
    def link_data_fun(self):
        ...
        self.logger.info(
            f"Processing file {self.ims_filename!s} stage 1/3...",
        )
        ...
        for i in range(0, len(channel_names)):
            ...
            contains_stat_type = (
                self.f["Scene8"]["Content"][current_channel]
            ).__contains__("StatisticsType")

            contains_stat_val = (
                self.f["Scene8"]["Content"][current_channel]
            ).__contains__("StatisticsValue")

            if contains_stat_type is True and contains_stat_val is True:
