...
class OmeroDwonload(CustomValidation):
    ...
    def _create_output_directory(self, name: str) -> Path:
        ...
        output = self.out_dir.joinpath(name)
        ...
        for d in data:
            if self.name or self.object_id:
                if (
                    ...
                    and d.getName() == self.name
                    or d.getId() == self.object_id
                ):
                    logger.info(
                    ...
                    for data in d.listChildren():
                        ...
                        for image in data.listChildren():
                            if image.getFileset() is None:
                                msg = f"Files are not downloaded \
                                {self.data_type}: name={dataset_name}"
