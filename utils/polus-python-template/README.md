...
There are two different base images that are used when this file is generated by cookiecutter: `labshare/polus-bfio-util` or `python:3.7-alpine`. If `use_bfio` is set to true, then the base image will be `labshare/polus-bfio-util`. This image comes loaded with Python 3 and Java, with the `bfio` and `javabridge` packages along with all jars needed to run `bioformats`.
