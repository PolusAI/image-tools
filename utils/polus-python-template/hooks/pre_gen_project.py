...
if not plugin_package.startswith("polus.plugins."):
    raise ValueError(
        "plugin package must be a child of polus.plugins."
        + f"plugin_package must start with 'polus.plugins'. Got : {plugin_package}",
    )
if plugin_package.endswith("_plugin"):
    msg = f"plugin_package must not ends with _plugin. Got : {plugin_package}"
    raise ValueError(
        msg,
    )
...
assert plugin_slug.startswith("polus-")
assert plugin_slug.endswith("-plugin")
