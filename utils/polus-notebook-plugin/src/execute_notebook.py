...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="script", description="Script to execute Jupyter Notebooks",
    )

    # Parse input arguments from WIPP format: '--PARAMETER VALUE'
    parser.add_argument(
        "--input-collection",
        dest="input_collection",
        type=str,
    ...
    logger.info(f"Input collection: {input_collection}")
    logger.info(f"Input notebook: {input_notebook}")
    logger.info(f"Config file: {config_file}")
    logger.info(f"Output collection: {output_collection}")
    ...
    if config_file is None:
        pm.execute_notebook(
            ...
            parameters={"input_path": input_collection, "output_path": output_collection},
        )
    else:
        pm.execute_notebook(
            input_notebook,
            ...
            engine_name="sos" if is_sos else None,
            parameters={
                "input_path": input_collection,
                "output_path": output_collection,
                "config_file_path": config_file,
            },
        )

    logger.info(
        f"Execution completed in {time.time() - process_start} seconds!",
    )
