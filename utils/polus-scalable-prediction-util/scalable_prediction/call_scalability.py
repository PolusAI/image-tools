...
# %%
def scalable_prediction(yxzct : tuple,
                        ...
                        step_size : tuple,
                        max_label : int,
                        segment_locations : dict,
                        skipped_segments  : list):

    """ This function is called after a prediction has been made to a tiled image. 
    It copies the prediction at the appropriate location depending on the yxzct inputs.
    
    The window_size must be bigger than the step_size for all dimensions so that
    the algorithm can piece together broken segments from the previous tile.  
    This is done by comparing where the broken segments overlap so that they can 
    identified as the same segments.

    When bits of the broken segments exists outside the current view of the global 
    output, then we go back to that tile using information saved in segment_locations 
    to make the appropriate updates. The Zarr file format allows bfio objects to read
    and written simultaneously.

    Args:
        yxzct - the location of the current tile.  Specifies the starting and 
        ending index for the yxzct dimensions.
        prediction - the prediction at the current tile
        biowriter_obj - the bfio object that holds the entire image
        zarr_image - the location of the output bfio object.  
                     This variable is used when accessing previous tiles that have 
                     already been saved to the global bfio object (biowriter_obj)
        window_size - the size of the tile at which the algorithm makes a prediction on.
        step_size - how much the tile slides over to make the next prediction on.
        max_label - The largest labeled segment that has been saved.  So that the next label 
                    saved in the global bfio object is in sequence to the previous label saved.
        segment_locations - stores information as to which each segment belong in every 
                            tile
        skipped_segments - segments that have been skipped over when relabeling broken bits
                          of segments.  The labels need to be in order and there can be 
                          no missing label in the output.
    Returns:
        max_label - the updated max_label after saving the current tile into the global object
        segment_locations - the updated segment locations after saving the current tile into 
                            the global object
        skipped_segments - segments that have been removed when relabelling segments outside 
                           the current view
    """
    y1, y2 = yxzct[0]
    x1, x2 = yxzct[1]
    ...
    if y1 == 0 and x1 == 0:
        ...
        out_view = np.zeros(pred_shape)
    else:
        with bfio.BioReader(zarr_image, max_workers=1) as read_bw:
            out_view = read_bw[y1:y2, x1:x2, :, :, :]
            out_view = np.reshape(out_view, pred_shape)
    
    # Gets all the pairs that overlap each other in the overlap strip.
    unique_pairs = np.unique(np.c_[out_view.flatten(), prediction.flatten()], axis=0)
    ...
    for new_lab in new_maps2_old.keys():
        ...
        if len(old_labs) == 1: 
            old_lab = old_labs[0]
            prediction[prediction == new_lab] = old_lab
        ...
        # with the smallest label among them. 
            ...
            prediction[prediction==new_lab] = mini
    ...
    # previous tile
    new_labels = np.unique(prediction)
    ...
    maxes = np.maximum(out_view, prediction)
    biowriter_obj[int(y1):int(y2), int(x1):int(x2), :, :, :] = maxes

    return segment_locations, skipped_segments, max_label

# %%
def main():

    model_path = './models'
    image_path = './IMAGE_dsb2018_x(0-7)_y(0-7).ome.tif'
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        temp_image_file = os.path.join(temp_dir, "padded.ome.tif")
        image_path, max_value, min_value = make_input_compatible(input_image  = image_path,
                                                                 output_image = temp_image_file, 
                                                                 tile_len     = tile_len)

        print(f'Random image chosen: {image_path}')
        print(f'Max Value in Image: {max_value}')
        print(f'Min Value in Image: {min_value}')

            # Not using GPU helps save memory
        os.environ["CUDA_VISIBLE_DEVICES"] = "-1"

        # Load the model
        model = SplineDist2D(None, name=model_path)
        ...
        with bfio.BioReader(image_path, max_workers=2) as br_image:
            ...
            # double check that input has the appropriate shape
            mod_image_shape = [shape%tile_len for shape in image_shape if shape != 1]
            for mod in mod_image_shape:
                if mod != 0:
                    raise ValueError("Image Shape needs to be in multiples of {} \
                                    \n Run add_padding() function to input image".format(tile_len))
            ...
            with bfio.BioWriter(zarr_image,
                ...
                # In order to keep track of what number label to start with
                max_label = 0
                # Segment locations is a dictionary that keeps track of where all
                # segment pieces can be found
                segment_locations = {}
                # This helps to keep track if segments are skipped over, so order
                # is maintained when relabeling to smaller label values
                skipped_segments = []

                for yxzct, tiled_image in sliding_window(br_image, image_shape, window_size, step_size):

                    tiled_image = normalize(tiled_image, pmin=1, pmax=99.8, axis=(0,1),dtype=int)
                    tiled_image_shape = tiled_image.shape

                    tiled_prediction, _ = model.predict_instances(tiled_image)
                    tiled_prediction = np.reshape(tiled_prediction, (tiled_image_shape[0], tiled_image_shape[1], 1, 1, 1))
                    tiled_prediction = tiled_prediction.astype(np.float64)
                    tiled_prediction[tiled_prediction > 0] = tiled_prediction[tiled_prediction > 0] + max_label 
                    
                    
                    del tiled_image
                    
                    segment_locations, skipped_segments, max_label = scalable_prediction(yxzct=yxzct, 
                                                                                        prediction=tiled_prediction,
