...
def validate_and_copy(
    ...
    with BioReader(file["file"], backend="python"):
...
def main(
    ...
    with ThreadPoolExecutor(cpu_count()) as executor:
        ...
        while len(not_done) > 0:
            ...
            logger.info(
                "Copy progress: {:6.2f}%".format(100 * len(done) / len(threads)),
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="main",
        description="Copies .ome.tif files with proper tile format from a generic data type to an image collection.",
    )
    ...
    main(inpDir=inpDir, outDir=outDir)
