...
if __name__ == "__main__":
    ...
    logger.debug(f"Plugins to Generate: {plugins_to_generate}")
    ...
    for plugin in plugins:
        if plugin.name in plugins_to_generate:
            ...
            path = polus_plugins_dir.joinpath(
                "polus-imagej-" + plugin.name.lower() + "-plugin",
            )
            ...
            os.system(
                "cookiecutter {} --output-dir {} --no-input".format(
                    str(base_path), str(polus_plugins_dir),
                ),
            )
            ...
            with open(shell_test_path, "a") as fhand:
                ...
                commands = [
                    "python "
                    + str(path)
                    + f"/tests/unit_test.py --opName '{op}'"
                    for op in op_methods
                ]

                # Generate the shell script lines
                lines = [f'os.system("{command}")\n' for command in commands]
    ...
    logger.debug(f"There were {pluging_count} plugins generated\n")
