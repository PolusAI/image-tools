...
class UnitTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        file_handler = logging.FileHandler('imagej-testing/data-test.log')
    ...
    def generate_data(self, input, wipp_type, imagej_type):
        ...
        # Determine if the input data type is a collection
            ...
            # Create input dir path object for the randomly generated images
            input_dir = Path(__file__).parent.joinpath(input)
            
            # Check if "input" directory is a sub-directory of "tests"
            if input_dir.exists():
            ...
            # Iterate over all numpy data types
            for data_name, data_type in numpy_types.items():
                
                array = data_type(random_image)
                
                # Define the image's file path
                image_path = input_dir.joinpath('random_{}.ome.tif'.format(data_name))
                        
                # Create a BioWriter object to write the ramdomly generated image file to tests/input dir
                with BioWriter(image_path) as writer:
                    writer.X = image_shape[0]
                    writer.Y = image_shape[1]
                    writer.dtype = data_type
                    writer[:] = array[:]
