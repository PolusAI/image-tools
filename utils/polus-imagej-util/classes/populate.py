...
class Op:
    ...
    def __init__(
        self, plugin: "Plugin", name: str, full_path: str, inputs: list, output: tuple,
    ) -> None:
        ...
        if not isinstance(inputs, list):
            msg = "inputs must be an instance of a list"
            raise TypeError(msg)
        ...
        self._inputs.extend(
            [
                (_input, Op.imagej_to_Wipp_map.get(_input[0], "unknown"))
    ...
    def __support(self):
        ...
        # Check if the input and output both contain collection data types
            ...
            "collection" not in self.wipp_type_inputs
    ...
    def add_op(self, op: "Op") -> None:
        ...
        if op.partial_support:
            ...
            for title, dtype, wippType in zip(
                op.imagej_title_required_inputs,
                ...
                # If variable key exists update it
                    ...
                    self._all_required_inputs[title]["wipp_type"].update(
                        {op.name: wippType},
                    )
                    self._all_required_inputs[title]["call_types"].update(
                        {op.name: dtype},
                    )
            ...
            if self._all_outputs == {}:
                ...
                self._all_outputs = {
                    op.imagej_title_output: {
                        ...
                        "description": "out",
                        "call_types": {op.name: op.imagej_type_output},
                    },
                }
            ...
            # Check if the output title is not in dictionary
                ...
                self._all_outputs[op.imagej_title_output]["call_types"][
                    op.name
                ] = op.imagej_type_output
...
class Populate:
    ...
    def __init__(
        ...
        with open(Path(__file__).parents[1].joinpath("scale.json")) as f:
    ...
    def _parser(self) -> None:
        ...
        for plugin in plugins:
            ...
            for op_doc in split_ops[1:]:
                ...
                support_msg = True if op.partial_support else op.support_msg
    ...
    def __logger(self, log_file: str, log_template: str) -> None:
            ...
            if len(plugin.supported_ops) > 0:
                ...
                self.json_dic[name] = {
                    ...
                    "project_short_description": str(
                        list(plugin.supported_ops.keys()),
                    ).replace("'", "")[1:-1],
                    "citation": "",
                    "plugin_namespace": {
                        op.name: "out = ij.op()."
...
class GeneratedParser:
    ...
    def __init__(self, polus_plugins_directory=None, cookiecutter_directory=None) -> None:
        ...
        manifest_paths = list(self.polus_plugins.rglob("polus-imagej*/plugin.json"))
        ...
        for p in manifest_paths:
            ...
            plugin_name = (
                str(p.parent.name).replace("polus-imagej-", "").replace("-plugin", "")
            )
        ...
        cookietin_paths = list(self.cookietin.rglob("cookiecutter.json"))
    ...
    def update_templates(self, ops="all", skip=[]):
        ...
        for plugin, template in self.cookiecutter.items():
            ...
            # Check if a plugin manifest exists for the current plugin
                ...
                print(f"{plugin} has no manifest")
                continue
            ...
            if "out_input" in template["_inputs"]:
                # Check if the input lengths match
                if len(template["_inputs"]) - 1 != len(manifest["inputs"]):
                    msg = f"The {plugin} template and manifest have a varying number of inputs"
            ...
            cookiecutter_path = self.cookietin.joinpath(plugin).joinpath(
                "cookiecutter.json",
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open(version_path) as fhand:
            version = next(fhand)
