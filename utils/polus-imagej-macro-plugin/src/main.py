...
def close_image(image_title: str, ij: object) -> None:
    ...
    if image_title in available_images:
        logger.debug(f"Closing {image_title}")
        logger.debug("Current availalbe images:")
        logger.debug(available_images)

        # Attempt to close the image via macro command
        ij.py.run_macro(jpype.JString(f'close("{image_title}");'))

    else:
        logger.debug(f"Cannot close {image_title} not in available images")


# Define method to repeatedly run macro
def run_macro(
    numpy_input: np.array, image_title: str, script: str, ij: object, maxIterations: int,
) -> np.array:
    ...
    while i < maxIterations:
        ...
        i += 1
        logger.debug(f"Running macro attempt {i}")
        ...
        try:
            ...
            java_output = ij.WindowManager.getImage(
                jpype.JString(image_title + "-output"),
            )
            ...
            if np.array_equal(numpy_input, numpy_output):
                logger.info(f"Macro attempt {i} failed")
                continue
            ...
            if i >= maxIterations:
                msg = f"Failed to run macro on image {image_title} after {i} attempts"
                raise Exception(
                    msg,
                )
...
def main(inpDir, macroDir, outDir, maxIterations):
    ...
    logger.debug(f"Is JVM running: {jpype.isJVMStarted()}")
    ...
    for app in apps:
        ...
        logger.info(f"Loaded {app} version {versions[app]}")
    ...
    for path in image_paths:
        ...
        logger.debug(f"Path stem is {path_stem}")
        ...
        assert not np.array_equal(
            numpy_input, numpy_output,
        ), "The input and output images are identical"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="main", description="ImageJ Macro Plugin")
    ...
    logger.info(f"inpDir = {_inpDir}")

    _macroDir = Path(args.macroDir)
    logger.info(f"macroDir = {_macroDir}")

    _outDir = Path(args.outDir)
    logger.info(f"outDir = {_outDir}")

    _maxIterations = args.maxIterations if args.maxIterations is not None else 10
    logger.info(f"maxIterations = {_maxIterations}")

    main(
        inpDir=_inpDir, macroDir=_macroDir, outDir=_outDir, maxIterations=_maxIterations,
    )
