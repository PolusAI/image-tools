...
file names and then parametrically loop over files. A `filepattern` is
essentially a simplified regular expression with named groups, and regular
expressions are valid `filepattern` expressions provided they do not use groups.
...
convention. This made abstracting things like image stitching algorithms easier
to apply to files with vastly disparate naming conventions by simply changing
the `filepattern` rather than generating new code to parse each new naming
