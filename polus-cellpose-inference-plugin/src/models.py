...
class Cellpose():
    ...
    def eval(self, x,image_name, channels=None, diameter=30., invert=False,  anisotropy=None,
        ...
        nimg = len(x)
        print('processing %s image(s)'%image_name)
        ...
        if diameter is not None and diameter!=0:
            ...
            if self.pretrained_size is not None and rescale is None :
                ...
                rescale = self.diam_mean / diams.copy()
                print('estimated cell diameters for %s image(s) in %0.2f sec'%(image_name, time.time()-tic))
            else:
                if rescale is None:
        ...
        loc,prob= self.cp.eval(x, invert=invert, rescale=rescale, anisotropy=anisotropy,
                                            ...
                                            net_avg=net_avg)
       # print('msks',loc.shape)
        print('Estimated probablity of cells   for %s image(s) in %0.2f sec'%(image_name, time.time()-tic))
