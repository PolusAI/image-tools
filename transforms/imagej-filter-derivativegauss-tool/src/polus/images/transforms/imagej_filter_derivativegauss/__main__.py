...
import tqdm
import typer
from polus.images.transforms.imagej_filter_derivativegauss import POLUS_LOG
from polus.images.transforms.imagej_filter_derivativegauss import filter_derivativegauss
...
def main(
    ...
    derivatives_: str = typer.Option(
        ...,
        "--derivatives",
        help="Comma separated integers for the nth derivatives",
    ),
    sigma_: str = typer.Option(
        ...,
        "--sigma",
        help="Comma separated floats for the sigma values along each axis",
    ),
    ...
    derivatives = [int(d) for d in derivatives_.split(",")]
    sigma = [float(s) for s in sigma_.split(",")]
    ...
    for inp_path in tqdm.tqdm(files):
        filter_derivativegauss(inp_path, derivatives, sigma, out_dir, ij)
