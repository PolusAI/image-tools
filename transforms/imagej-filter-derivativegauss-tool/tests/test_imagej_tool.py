...
from polus.images.transforms.imagej_filter_derivativegauss.__main__ import app
from polus.images.transforms.imagej_filter_derivativegauss.__main__ import main
...
def test_imagej_tool(
    ...
    """Test the tool."""
    inp_dir, _, out_dir = gen_data

    main(inp_dir, ".*", "2,2", "1.5,1.5", out_dir)
...
# @pytest.mark.skip(reason="CLI tests are not working")
def test_cli(
    gen_data: tuple[pathlib.Path, pathlib.Path, pathlib.Path],
) -> None:
    """Test the CLI."""
