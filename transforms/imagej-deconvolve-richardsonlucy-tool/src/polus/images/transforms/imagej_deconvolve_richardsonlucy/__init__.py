...
logger = logging.getLogger("polus.images.transforms.imagej_deconvolve_richardsonlucy")
logger.setLevel(POLUS_LOG)


def deconvolve_richardsonlucy(
    ...
    logger.info(f"{ij.op().help('deconvolve.richardsonLucy')}")
            ...
            for y_min in range(0, br_img.Y, TILE_SIZE):
                ...
                for x_min in range(0, br_img.X, TILE_SIZE):
                    ...
                    img_tile = (
                        ij.op()
                        .deconvolve()
                        .richardsonLucy(out_tile, img_tile, psf_tile, max_iterations)
                    )
