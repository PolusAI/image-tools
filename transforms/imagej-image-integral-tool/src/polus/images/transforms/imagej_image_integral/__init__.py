"""Provides the ImageJ image integral tool."""
...
logger = logging.getLogger("polus.images.transforms.imagej_image_integral")
...
def image_integral(
    ...
    logger.info(f"{ij.op().help('image.integral')}")
        ...
        with bfio.BioWriter(out_path, metadata=br_img.metadata) as bw:
            ...
            for y_min in range(0, br_img.Y, TILE_SIZE):
                ...
                for x_min in range(0, br_img.X, TILE_SIZE):
                    ...
                    img_tile = ij.op().image().integral(img_tile)
