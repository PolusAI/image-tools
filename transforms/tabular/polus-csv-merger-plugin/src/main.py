...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="stripExtension",
        type=str,
        help="Strip csv extension in the column designating file origin for a row",
        required=True,
    )
    parser.add_argument(
        "--outDir", dest="outDir", type=str, help="Output csv file", required=True
    )
    parser.add_argument(
        "--dim", dest="dim", type=str, help="rows or columns", required=True
    )
    parser.add_argument(
        "--sameRows",
        dest="sameRows",
        type=bool,
        help="Only merge csvs if they contain the same number of rows",
        default=False,
        required=False,
    )
    parser.add_argument(
        ...
        dest="skipChecks",
        type=bool,
        help="Skip header checks when merging along rows.",
        default=False,
        required=False,
    )
    ...
    same_rows = args.sameRows
    logger.info("sameRows = {}".format(stripExtension))
    skip_checks = args.skipChecks
    logger.info("sameRows = {}".format(skip_checks))

    # Get input files
    inpDir_files = [
        str(f.absolute()) for f in Path(inpDir).iterdir() if f.name.endswith(".csv")
    ]
    ...
    if dim == "columns" and same_rows:
        ...
        for key in out_files.keys():
            ...
            outPath = str(
                Path(outDir).joinpath("merged_{}.csv".format(count)).absolute()
            )
            ...
            if FILE_EXT == ".feather":
                ...
                for l in range(key):
                    for f in inpDir_files:
                        ...
                        dfs.append(df)
                        df_final = ft.reduce(
                            lambda left, right: pd.merge(left, right), dfs
                        )
        ...
        if skip_checks and dim == "rows":
            with open(inpDir_files[0], "r") as fr:

                # Get header information
                line = fr.readline()

                h = line.rstrip("\n").split(",")
                headers_set.update(h)
        ...
        if "file" in headers_set:
            headers_set.remove("file")
        headers: list = list(headers_set)
        headers.sort()
        if not (skip_checks and dim == "rows"):
        ...
        line_template: str = ",".join([("{" + h + "}") for h in headers]) + "\n"
        line_dict = {key: "NaN" for key in headers}

        # Generate the path to the output file
        outPath = (
            str(Path(outDir).joinpath("merged.feather").absolute())
            if FILE_EXT == "feather"
            else str(Path(outDir).joinpath("merged.csv").absolute())
        )
            ...
            with open(outPath, "w") as out_file:
                ...
                for f in inpDir_files:
                    ...
                    with open(f, "r") as in_file:
                        ...
                        # Check to see if column identifiers are present in the
                        # file, skip 2nd row if they are present
                        if identifiers:
            ...
            with open(inpDir_files[0], "r") as in_file:
                ...
                for line in in_file:
                    ...
                    if file_dict["file"] in out_dict.keys():
                        UserWarning(
                            "Skipping row for file since it is already in the output file dictionary: {}".format(
                                file_dict["file"]
                            )
                        )
                    else:
                    ...
                    for line in in_file:
                        ...
                        for el, val in enumerate(line_vals):
                            if line_vals[file_ind] not in out_dict.keys():
                                ...
                                out_dict[line_vals[file_ind]]["file"] = line_vals[
                                    file_ind
                                ]
                            if el == file_ind:
                                continue
                            if out_dict[line_vals[file_ind]][file_map[el]] != "NaN":
                                Warning(
                                    "Skipping duplicate value ({}) found in {}".format(
                                        val, in_file
                                    )
                                )
                            else:
