...
import tqdm
import typer
from polus.images.transforms.imagej_filter_frangivesselness import POLUS_LOG
from polus.images.transforms.imagej_filter_frangivesselness import (
    filter_frangivesselness,
)
...
def main(
    ...
    spacing_: str = typer.Option(
        ...,
        "--spacing",
        help=(
            "The physical distance between data points in the image, can vary "
            "for each dimension of the image. Single value or comma "
            "separated list of values"
        ),
    ),
    scale: int = typer.Option(..., "--scale", help="The Frangi scale parameter."),
    ...
    if "," in spacing_:
        spacing = list(map(float, spacing_.split(",")))
    else:
    ...
    for inp_path in tqdm.tqdm(files):
        filter_frangivesselness(inp_path, spacing, scale, out_dir, ij)
