...
class PolygonSet:
            ...
            for z in range(0, reader.Z, tile_size):
                ...
                for y in range(0, reader.Y, tile_size):
                    ...
                    for x in range(0, reader.X, tile_size):
                        ...
                        tile_count += 1
                        logger.debug(
                            f"added tile #{tile_count} ({z}:{z_max}, {y}:{y_max}, {x}:{x_max})",
                        )
                    logger.info(
                        f"Reading Progress {100 * tile_count / (num_slices * num_cols * num_rows):6.3f}%...",
                    )
    ...
    def write_to(self, outfile: Path):
        ...
        """
        with BioWriter(
            outfile, metadata=self.metadata, max_workers=cpu_count(),
        ) as writer:
            ...
            for z in range(writer.Z):
                for y in range(0, writer.Y, tile_size):
                    ...
                    for x in range(0, writer.X, tile_size):
                        ...
                        tile = extract_tile(
                            self.__polygon_set, (z, z + 1, y, y_max, x, x_max),
                        )
                ...
                logger.info(
                    f"Writing Progress {100 * tile_count / (num_cols * num_rows * writer.Z):6.3f}%...",
                )
        return self
