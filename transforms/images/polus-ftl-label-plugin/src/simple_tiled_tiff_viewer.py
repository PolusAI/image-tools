"""Simple tiled TIFF viewer for comparing input and FTL label outputs."""
import argparse
import logging
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="main",
        description="View *.ome.tif images and labels from FTL plugin.",
    )
    parser.add_argument(
        "--inpDir",
        dest="inpDir",
    ...
    files = [
        f for f in input_dir.iterdir() if f.is_file() and f.name.endswith(".tif")
    ]

    for file in files:
        # Set up the BioReader
        with BioReader(input_dir / file.name) as br_in:
            img_in = br_in[:]

        with BioReader(output_dir / file.name) as br_out:
            img_out = br_out[:]
