...
class UnitTest(unittest.TestCase):
    ...
    def test_plugin(self):
        ...
        projectName = "ImageJ filter partialDerivative"
        self.logger.info(
        ...
        _opName = op
        _in1_wipp_types = {"PartialDerivativeRAI": "collection"}
        _in1_imagej_types = {"PartialDerivativeRAI": "RandomAccessibleInterval"}
        ...
        _dimension_wipp_types = {"PartialDerivativeRAI": "number"}
        _dimension_imagej_types = {"PartialDerivativeRAI": "int"}
        if _dimension_wipp_types.get(op, None) != "collection":
            method_call_types.update(
                {
                    method: dtype
                    for method, dtype in _dimension_imagej_types.items()
                    if dtype in supported_data_types
                }
            )
        ...
        _dimension = self.generate_data(
            "dimension",
            _dimension_wipp_types.get(op, None),
        ...
        try:
            # Call the op
            main(_opName=_opName, _in1=_in1, _dimension=_dimension, _out=_out)
            self.logger.info(
