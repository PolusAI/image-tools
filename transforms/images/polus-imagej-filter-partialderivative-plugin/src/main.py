...
def main(
    ...
    # Validate in1
    in1_types = {
        "PartialDerivativeRAI": "RandomAccessibleInterval",
    }
    ...
    # Validate dimension
    dimension_types = {
        "PartialDerivativeRAI": "int",
    }

    # Check that all inputs are specified
    if _dimension is None and _opName in list(dimension_types.keys()):
        raise ValueError("{} must be defined to run {}.".format("dimension", _opName))
    else:
        dimension = None
    ...
    try:
        for ind, (in1_path,) in enumerate(zip(*args)):
            ...
            if _dimension is not None:
                dimension = ij_converter.to_java(
                    ij, _dimension, dimension_types[_opName], dtype
                )

            logger.info("Running op...")
            if _opName == "PartialDerivativeRAI":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="dimension",
        type=str,
        help="Dimension in which the partial derivative should be applied",
        required=False,
    )
    ...
    logger.info("dimension = {}".format(_dimension))
