...
class UnitTest(unittest.TestCase):
    ...
    def test_plugin(self):
        ...
        projectName = "ImageJ deconvolve richardsonLucyCorrection"
        self.logger.info(
        ...
        _opName = op
        _in1_wipp_types = {"RichardsonLucyCorrection": "collection"}
        _in1_imagej_types = {"RichardsonLucyCorrection": "RandomAccessibleInterval"}
        ...
        _in2_wipp_types = {"RichardsonLucyCorrection": "collection"}
        _in2_imagej_types = {"RichardsonLucyCorrection": "RandomAccessibleInterval"}
        ...
        _fftBuffer_wipp_types = {"RichardsonLucyCorrection": "collection"}
        _fftBuffer_imagej_types = {
            "RichardsonLucyCorrection": "RandomAccessibleInterval"
        }
        if _fftBuffer_wipp_types.get(op, None) != "collection":
            method_call_types.update(
                {
                    method: dtype
                    for method, dtype in _fftBuffer_imagej_types.items()
                    if dtype in supported_data_types
                }
            )
        _fftKernel_wipp_types = {"RichardsonLucyCorrection": "collection"}
        _fftKernel_imagej_types = {
            "RichardsonLucyCorrection": "RandomAccessibleInterval"
        }
        if _fftKernel_wipp_types.get(op, None) != "collection":
            method_call_types.update(
                {
                    method: dtype
                    for method, dtype in _fftKernel_imagej_types.items()
                    if dtype in supported_data_types
                }
            )
        ...
        _fftBuffer = self.generate_data(
            "fftBuffer",
            _fftBuffer_wipp_types.get(op, None),
            method_call_types.get(op, None),
        )
        _fftKernel = self.generate_data(
            "fftKernel",
            _fftKernel_wipp_types.get(op, None),
            method_call_types.get(op, None),
        )
