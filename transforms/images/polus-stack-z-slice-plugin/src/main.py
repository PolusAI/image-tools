...
def _merge_layers(input_files, output_path):
    with ProcessManager.process(output_path.name):
        ...
        with BioWriter(
            output_path, metadata=metadata, max_workers=ProcessManager._active_threads,
        ) as bw:
            ...
            for file in input_files:
                # Open an image
                with BioReader(
                    file["file"], max_workers=ProcessManager._active_threads,
                ) as br:
...
def main(input_dir: pathlib.Path, file_pattern: str, output_dir: pathlib.Path) -> None:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="main",
        description="Compile individual tiled tiff images into a single volumetric tiled tiff.",
    )
