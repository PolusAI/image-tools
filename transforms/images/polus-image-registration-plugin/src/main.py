...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="main", description="This plugin registers an image collection",
    )
    parser.add_argument(
        "--filePattern",
        dest="filePattern",
    ...
    parser.add_argument(
        ...
        type=str,
        help="variable to help identify which images need to be registered to each other",
        required=True,
    )
    parser.add_argument(
        "--template",
    ...
    logger.info(f"registrationVariable = {registrationVariable}")
    template = args.template
    logger.info(f"template = {template}")
    TransformationVariable = args.TransformationVariable
    logger.info(f"TransformationVariable = {TransformationVariable}")
    outDir = args.outDir
    ...
    for registration_set, similar_transformation_set in registration_dictionary.items():
        ...
        if registration_set[0] == registration_set[1]:
            ...
            for image_path in similar_transformation_set:
                ...
                shutil.copy2(
                    image_path, str(Path(outDir).joinpath(image_name).absolute()),
                )
            continue
        ...
        registration = subprocess.Popen(
            "python3 image_registration.py --registrationString '{}' --similarTransformationString '{}' --outDir '{}' --template '{}' --method '{}'".format(
                registration_string,
