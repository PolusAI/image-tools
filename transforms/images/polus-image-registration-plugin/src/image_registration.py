    ...
    with ThreadPoolExecutor(max([cpu_count() // 2, 1])) as executor:
        for x in range(0, image.num_x(), stride):
            ...
            for y in range(0, image.num_y(), stride):
                ...
                threads.append(
                    executor.submit(
                        load_and_scale, [x, x_max], [y, y_max], [xi, xe], [yi, ye],
                    ),
                )
...
def register_image(
    br_ref, br_mov, bw, Xt, Yt, Xm, Ym, x, y, X_crop, Y_crop, max_val, min_val, method,
):
    ...
    ref_tile = br_ref.read_image(
        X=[Xt[0], Xt[1]], Y=[Yt[0], Yt[1]], Z=[0, 1], C=[0], T=[0],
    ).squeeze()
    mov_tile = br_mov.read_image(
        X=[Xm[0], Xm[1]], Y=[Ym[0], Ym[1]], Z=[0, 1], C=[0], T=[0],
    ).squeeze()
    ...
    if method == "Projective":
        transformed_image = cv2.warpPerspective(
            mov_tile, projective_transform, (Xt[1] - Xt[0], Yt[1] - Yt[0]),
        )
    else:
    ...
    if corr < 0.4 and not is_rough:
        if method == "Projective":
            transformed_image = cv2.warpPerspective(
                mov_tile, Rough_Homography_Upscaled, (Xt[1] - Xt[0], Yt[1] - Yt[0]),
            )
        else:
            transformed_image = cv2.warpAffine(
                mov_tile, Rough_Homography_Upscaled, (Xt[1] - Xt[0], Yt[1] - Yt[0]),
...
def apply_transform(br_mov, bw, tiles, shape, transform, method):
    ...
    if method == "Projective":
        transformed_image = cv2.warpPerspective(
            mov_tile, transform, (Xt[1] - Xt[0], Yt[1] - Yt[0]),
        )
    else:
        transformed_image = cv2.warpAffine(
            mov_tile, transform, (Xt[1] - Xt[0], Yt[1] - Yt[0]),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--registrationString", dest="registration_string", type=str, required=True,
    )
    parser.add_argument(
    ...
    reference_image_downscaled, max_val, min_val = get_scaled_down_images(
        br_ref, scale_factor, get_max=True,
    )
    ...
    Rough_Homography_Downscaled = get_transform(
        moving_image_downscaled, reference_image_downscaled, max_val, min_val, method,
    )
    ...
    bw = BioWriter(
        str(Path(outDir).joinpath(Path(registration_set[1]).name)),
    ...
    for moving_image_path in similar_transformation_set:
        ...
        bw = BioWriter(
            str(Path(outDir).joinpath(moving_image_name)),
            metadata=br_mov.read_metadata(),
            max_workers=write_workers,
        )
        bw.num_x(br_ref.num_x())
        ...
        with ThreadPoolExecutor(loop_workers) as executor:
            ...
            for tile, shape, transform in zip(reg_tiles, reg_shape, reg_homography):
                ...
                threads.append(
                    executor.submit(
                        apply_transform, br_mov, bw, tile, shape, transform, method,
                    ),
                )
