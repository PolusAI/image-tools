"""CLI for autocropping 2D/3D images by entropy-based bounding box estimation."""
import argparse
import logging
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="main",
        description=(
            "Autocropping 2d and 3d images by estimating entropy of "
            "rows, columns, and z-slices."
        ),
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Variables to use for grouping images. Each group is cropped "
    ...
    parser.add_argument(
        ...
        default="true",
        help=(
            "Whether to use gaussian smoothing on images to add more "
            "tolerance to noise."
        ),
    )
    ...
    if len(error_messages) > 0:
        ...
        for i, group in enumerate(groups):
            ...
            logger.info(
                f"Working on group {i + 1}/{len(groups)} "
                f"containing {len(file_paths)} images...",
            )
            crop_image_group(
