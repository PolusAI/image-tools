...
def calculate_strip_entropy(
    *,
        ...
        for x_min, x_max, y_min, y_max in helpers.iter_strip(
            ...
            if smoothing:
                tile = scipy.ndimage.gaussian_filter(
                    tile, sigma=1, mode="constant", cval=numpy.mean(tile),
                )
    ...
    histograms[-1].extend(
        [
            distogram.Distogram(
                bin_count=constants.MAX_BINS, weighted_diff=constants.WEIGHTED_BINS,
            )
    ...
    strip_entropy: list[float] = [
        scipy.stats.entropy([c for _, c in histogram.bins]) for histogram in histograms
    ]
...
def find_gradient_spike_xy(
    ...
    for i, index in enumerate(strip_indices[:deepest_strip]):
        ...
        threshold = numpy.percentile(
            smoothed_gradients, q=constants.GRADIENT_PERCENTILE,
        )
...
def write_cropped_images(
    ...
    for file_path in file_paths:
        ...
        with BioReader(file_path) as reader:
            with BioWriter(
                out_path, metadata=reader.metadata, max_workers=constants.NUM_THREADS,
            ) as writer:
                writer.Z = out_depth
                writer.Y = out_width
                writer.X = out_height
                ...
                for z_out in range(writer.Z):
                    ...
                    for out_y in range(0, writer.Y, constants.TILE_STRIDE):
                        ...
                        for out_x in range(0, writer.X, constants.TILE_STRIDE):
                            ...
                            try:
                                tile = reader[
                                    in_y:in_y_max, in_x:in_x_max, z_in : z_in + 1, 0, 0,
                                ]
                                writer[
                                    out_y:out_y_max,
                                    out_x:out_x_max,
