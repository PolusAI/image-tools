...
def main(
    ...
    # Validate in1
    in1_types = {
        "GaussRAISingleSigma": "RandomAccessibleInterval",
        "DefaultGaussRAI": "RandomAccessibleInterval",
    }
    ...
    # Validate sigmas
    sigmas_types = {
        "DefaultGaussRAI": "double[]",
    }

    # Check that all inputs are specified
    if _sigmas is None and _opName in list(sigmas_types.keys()):
        raise ValueError("{} must be defined to run {}.".format("sigmas", _opName))
    else:
        sigmas = None
    ...
    try:
        for ind, (in1_path,) in enumerate(zip(*args)):
            ...
            if _sigmas is not None:
                sigmas = ij_converter.to_java(ij, _sigmas, sigmas_types[_opName], dtype)

            logger.info("Running op...")
            if _opName == "GaussRAISingleSigma":
                out = ij.op().filter().gauss(in1, sigma)
            elif _opName == "DefaultGaussRAI":
                out = ij.op().filter().gauss(in1, sigmas)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="opName",
        type=str,
        help="Operation overloading method to perform",
        required=False,
    )
    parser.add_argument(
        "--inpDir",
    ...
    parser.add_argument(
        ...
        dest="sigma",
        type=str,
        help="The standard deviation of the Gaussian filter, same in both dimensions.",
        required=False,
    )
    parser.add_argument(
        "--sigmas",
        dest="sigmas",
        type=str,
        help="The standard deviations of the Gaussian filters, (x,y)",
        required=False,
    )
    ...
    _sigmas = args.sigmas
    logger.info("sigmas = {}".format(_sigmas))

    # Output Args
    _out = Path(args.out)
    logger.info("outDir = {}".format(_out))

    main(_opName=_opName, _in1=_in1, _sigma=_sigma, _sigmas=_sigmas, _out=_out)
