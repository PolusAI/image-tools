...
class PolygonSet:
    """Creates a PolygonSet interface for the Rust implementation."""
    ...
    def __init__(self, connectivity: int) -> None:
        ...
        """
        if not (MIN_CONNECTIVITY <= connectivity <= MAX_CONNECTIVITY):
            msg = f"connectivity must be 1, 2 or 3. Got {connectivity} instead"
            raise ValueError(
                msg,
            )
            ...
            for z in range(0, reader.Z, tile_size):
                ...
                for y in range(0, reader.Y, tile_size):
                    ...
                    for x in range(0, reader.X, tile_size):
                        ...
                        tile_count += 1
                        msg = f"tile#{tile_count} ({z}-{z_max},{y}-{y_max},{x}-{x_max})"
                        logger.debug(msg)
                    msg = (
                        f"Reading {100*tile_count/(num_slices*num_cols*num_rows):6.2f}%"
                    )
                    logger.info(msg)
    ...
    def write_to(self, outfile: Path) -> "PolygonSet":
        ...
        Uses input metadata and sets dtype based on the number of label objects.
            ...
            for z in range(writer.Z):
                for y in range(0, writer.Y, tile_size):
                    ...
                    for x in range(0, writer.X, tile_size):
                        ...
                        tile_count += 1
                        logger.debug(
                            f"Wrote tile {tile_count}, ({z}, {y}:{y_max}, {x}:{x_max})",
                        )
                    msg = f"{100*tile_count/(num_cols*num_rows*writer.Z):.2f}% done"
                logger.info(msg)
        return self
