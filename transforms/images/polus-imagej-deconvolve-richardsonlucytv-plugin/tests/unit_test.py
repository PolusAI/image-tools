...
class UnitTest(unittest.TestCase):
    ...
    def test_plugin(self):
        ...
        projectName = "ImageJ deconvolve richardsonLucyTV"
        self.logger.info(
        ...
        _opName = op
        _in1_wipp_types = {"PadAndRichardsonLucyTV": "collection"}
        _in1_imagej_types = {"PadAndRichardsonLucyTV": "RandomAccessibleInterval"}
        ...
        _in2_wipp_types = {"PadAndRichardsonLucyTV": "collection"}
        _in2_imagej_types = {"PadAndRichardsonLucyTV": "RandomAccessibleInterval"}
        ...
        _maxIterations_wipp_types = {"PadAndRichardsonLucyTV": "number"}
        _maxIterations_imagej_types = {"PadAndRichardsonLucyTV": "int"}
        if _maxIterations_wipp_types.get(op, None) != "collection":
            method_call_types.update(
                {
                    method: dtype
                    for method, dtype in _maxIterations_imagej_types.items()
                    if dtype in supported_data_types
                }
            )
        _regularizationFactor_wipp_types = {"PadAndRichardsonLucyTV": "number"}
        _regularizationFactor_imagej_types = {"PadAndRichardsonLucyTV": "float"}
        if _regularizationFactor_wipp_types.get(op, None) != "collection":
            method_call_types.update(
                {
                    method: dtype
                    for method, dtype in _regularizationFactor_imagej_types.items()
                    if dtype in supported_data_types
                }
            )
        ...
        _maxIterations = self.generate_data(
            "maxIterations",
            _maxIterations_wipp_types.get(op, None),
            method_call_types.get(op, None),
        )
        _regularizationFactor = self.generate_data(
            "regularizationFactor",
            _regularizationFactor_wipp_types.get(op, None),
            method_call_types.get(op, None),
        )
