...
def main(
    ...
    # Validate in1
    in1_types = {
        "PadAndRichardsonLucy": "RandomAccessibleInterval",
        "RichardsonLucyC": "RandomAccessibleInterval",
    }
    ...
    maxIterations_types = {
        "PadAndRichardsonLucy": "int",
        "RichardsonLucyC": "int",
    }

    # Check that all inputs are specified
    if _maxIterations is None and _opName in list(maxIterations_types.keys()):
        raise ValueError(
            "{} must be defined to run {}.".format("maxIterations", _opName)
        )
    else:
        maxIterations = None
    ...
    try:
        for ind, (
            ...
            if _maxIterations is not None:
                maxIterations = ij_converter.to_java(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="in1",
        type=str,
        help="Input collection to be processed by this plugin",
        required=False,
    )
    parser.add_argument(
        "--psf",
        dest="in2",
        type=str,
        help="The point spread function mask to be used",
        required=False,
    )
    parser.add_argument(
        "--maxIterations",
        dest="maxIterations",
        type=str,
        help="The maximum number of algorithm iterations",
        required=False,
    )

    # Add command-line argument for each of the output arguments
    parser.add_argument(
        "--outDir", dest="out", type=str, help="The output collection", required=True
    )
    ...
    main(
        _opName=_opName, _in1=_in1, _in2=_in2, _maxIterations=_maxIterations, _out=_out
    )
