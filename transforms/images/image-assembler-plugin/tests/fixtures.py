...
def get_temp_file(path: Path, suffix: str) -> Path:
    ...
    name = "".join(random.choices("abcdefghijklmnopqrstuvwxyz", k=20))  # noqa: S311
    name += "".join(
        random.choices("abcdefghijklmnopqrstuvwxyz0123456789", k=20),  # noqa: S311
    )
    name += suffix
...
def data(  # noqa: PT004
    ...
    for offset in offsets:
        image_file: Path = img_path.joinpath(str(offset["file"]))
