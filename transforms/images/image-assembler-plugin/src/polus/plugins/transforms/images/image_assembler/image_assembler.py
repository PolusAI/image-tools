...
def derive_output_image_path(
    ...
    if derive_name_from_vector_file:
        ...
        if match is None:
            msg = f"the stitching vector file name does not match the expected pattern: {global_regex}"  # noqa: E501
            logger.critical(msg)
            raise ValueError(msg)

        match_names = match.groups()[0]
        if match_names:
            output_name = "".join([match_names, *Path(first_image).suffixes])

    return output_path / output_name


def assemble_image(  # noqa: PLR0915
    vector_file: Path,
    pattern: str,
    derive_name_from_vector_file: Optional[bool],
    img_path: Path,
    ...
    chunks: list = [[[] for _ in range(chunk_grid_col)] for _ in range(chunk_grid_row)]
    ...
    for fov in fovs():
        ...
        if len(fov[1]) != 1:
            msg = f"stitching vector contains multiple records for the same fov: {fov}"
            logger.critical(msg)
            raise ValueError(msg)
        filename: Path = fov[1][0]

        # get global coordinates of fov in the final image
        metadata = fov[0]
        global_fov_start_x: int = metadata["posX"]
        global_fov_start_y: int = metadata["posY"]
