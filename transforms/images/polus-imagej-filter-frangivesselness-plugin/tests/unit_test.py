...
class UnitTest(unittest.TestCase):
    ...
    def test_plugin(self):
        ...
        projectName = "ImageJ filter frangiVesselness"
        self.logger.info(
        ...
        _opName = op
        _in1_wipp_types = {"DefaultFrangi": "collection"}
        _in1_imagej_types = {"DefaultFrangi": "RandomAccessibleInterval"}
        ...
        _spacing_wipp_types = {"DefaultFrangi": "array"}
        _spacing_imagej_types = {"DefaultFrangi": "double[]"}
        if _spacing_wipp_types.get(op, None) != "collection":
            method_call_types.update(
                {
                    method: dtype
                    for method, dtype in _spacing_imagej_types.items()
                    if dtype in supported_data_types
                }
            )
        _scale_wipp_types = {"DefaultFrangi": "number"}
        _scale_imagej_types = {"DefaultFrangi": "int"}
        if _scale_wipp_types.get(op, None) != "collection":
            method_call_types.update(
                {
                    method: dtype
                    for method, dtype in _scale_imagej_types.items()
                    if dtype in supported_data_types
                }
            )
        ...
        _spacing = self.generate_data(
            "spacing",
            _spacing_wipp_types.get(op, None),
            method_call_types.get(op, None),
        )
        _scale = self.generate_data(
            "scale", _scale_wipp_types.get(op, None), method_call_types.get(op, None)
        )
        ...
        try:
            # Call the op
            main(
                _opName=_opName, _in1=_in1, _spacing=_spacing, _scale=_scale, _out=_out
            )
            self.logger.info(
