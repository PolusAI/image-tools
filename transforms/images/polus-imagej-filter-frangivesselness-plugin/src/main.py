...
def main(
    ...
    # Validate in1
    in1_types = {
        "DefaultFrangi": "RandomAccessibleInterval",
    }
    ...
    # Validate spacing
    spacing_types = {
        "DefaultFrangi": "double[]",
    }

    # Check that all inputs are specified
    if _spacing is None and _opName in list(spacing_types.keys()):
        raise ValueError("{} must be defined to run {}.".format("spacing", _opName))
    else:
        spacing = None
    ...
    # Check that all inputs are specified
    if _scale is None and _opName in list(scale_types.keys()):
        raise ValueError("{} must be defined to run {}.".format("scale", _opName))
    else:
        scale = None
    ...
    try:
        for ind, (in1_path,) in enumerate(zip(*args)):
            ...
            if _spacing is not None:
                spacing = ij_converter.to_java(
                    ij, _spacing, spacing_types[_opName], dtype
                )

            if _scale is not None:
                scale = ij_converter.to_java(ij, _scale, scale_types[_opName], dtype)
            ...
            if _opName == "DefaultFrangi":
                out = ij.op().filter().frangiVesselness(out_input, in1, spacing, scale)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="spacing",
        type=str,
        help="The physical distance between data points in the  image, can vary for each dimension of the image.",
        required=False,
    )
    parser.add_argument(
        "--scale",
    ...
    logger.info("spacing = {}".format(_spacing))

    _scale = args.scale
    logger.info("scale = {}".format(_scale))

    # Output Args
    _out = Path(args.out)
    logger.info("outDir = {}".format(_out))

    main(_opName=_opName, _in1=_in1, _spacing=_spacing, _scale=_scale, _out=_out)
