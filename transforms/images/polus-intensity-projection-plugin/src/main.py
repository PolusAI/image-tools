...
def max_min_projection(br, bw, x_range, y_range, max_workers, **kwargs):
    """Calculate the max or min intensity projection of a section of the input image.
    ...
    Args:
        ...
        max_workers: number of workers for reader/writer
    """
    br.max_workers = max_workers
    bw.max_workers = max_workers

    # set projection method
    method = kwargs.get("method", np.max)
...
def mean_projection(br, bw, x_range, y_range, max_workers, **kwargs):
    ...
    for ind, z in enumerate(range(0, br.Z, tile_size_z)):
        z_max = min([br.Z, z + tile_size_z])
        out_image += np.sum(
            br[y:y_max, x:x_max, z:z_max, ...].astype(np.float64), axis=2
        ).squeeze()
...
def process_image(input_img_path, output_img_path, projection, method):
    ...
    with BioReader(
        input_img_path, max_workers=max_workers
    ) as br, BioWriter(
        output_img_path, metadata=br.metadata, max_workers=max_workers
    ) as bw:
        # output image is 2d
        bw.Z = 1
            ...
            for x in range(0, br.X, tile_size):
                ...
                for y in range(0, br.Y, tile_size):
                    ...
                    if method is not None:
                        futures.append(
                            executor.submit(
                                projection, br, bw, (x, x_max), (y, y_max), max_workers, method=method
                            )
                        )
                    else:
                        futures.append(
                            executor.submit(
                                projection, br, bw, (x, x_max), (y, y_max), max_workers
                            )
                        )
            for f in futures:
                f.result()
    ...
    try:
        with ProcessPoolExecutor() as executor:
            futures = [
                executor.submit(
                    process_image,
                    os.path.join(inpDir, image_name),
