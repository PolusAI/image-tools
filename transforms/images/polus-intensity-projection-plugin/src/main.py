...
def max_min_projection(br, bw, x_range, y_range, **kwargs):
    ...
    with ProcessManager.thread():
        ...
        method = np.max if "method" not in kwargs else kwargs["method"]
...
def process_image(input_img_path, output_img_path, projection, method):
    ...
    with ProcessManager.process():
        ...
        with BioReader(
            input_img_path, max_workers=ProcessManager._active_threads,
        ) as br, BioWriter(
            output_img_path,
            ...
            for x in range(0, br.X, tile_size):
                ...
                for y in range(0, br.Y, tile_size):
                    ...
                    ProcessManager.submit_thread(
                        projection, br, bw, (x, x_max), (y, y_max), method=method,
                    )
    ...
    try:
        for image_name in inpDir_files:
            ...
            ProcessManager.submit_process(
                process_image, input_img_path, output_img_path, projection, method,
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="main", description="Calculate volumetric intensity projections",
    )
    ...
    logger.info(f"projectionType = {projectionType}")
    outDir = args.outDir
