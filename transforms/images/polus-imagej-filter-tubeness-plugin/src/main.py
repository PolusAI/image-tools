...
def main(
    ...
    if _calibration is None and _opName in list(calibration_types.keys()):
        raise ValueError("{} must be defined to run {}.".format("calibration", _opName))
    else:
        calibration = None
    ...
    try:
        for ind, (in1_path,) in enumerate(zip(*args)):
            ...
            if _calibration is not None:
                calibration = ij_converter.to_java(
                    ij, _calibration, calibration_types[_opName], dtype
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="sigma",
        type=str,
        help="Desired scale in physical units",
        required=False,
    )
    parser.add_argument(
        "--calibration",
        dest="calibration",
        type=str,
        help="Physical pixel sizes in all dimensions",
        required=False,
    )

    # Add command-line argument for each of the output arguments
    parser.add_argument(
        "--outDir", dest="out", type=str, help="Output directory", required=True
    )
    ...
    _calibration = args.calibration
    logger.info("calibration = {}".format(_calibration))
    ...
    main(
        _opName=_opName, _in1=_in1, _sigma=_sigma, _calibration=_calibration, _out=_out
    )
