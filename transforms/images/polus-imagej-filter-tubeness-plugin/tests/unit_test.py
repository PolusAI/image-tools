...
class UnitTest(unittest.TestCase):
    ...
    def test_plugin(self):
        ...
        projectName = "ImageJ filter tubeness"
        self.logger.info(
        ...
        _opName = op
        _in1_wipp_types = {"DefaultTubeness": "collection"}
        _in1_imagej_types = {"DefaultTubeness": "RandomAccessibleInterval"}
        ...
        _sigma_wipp_types = {"DefaultTubeness": "number"}
        _sigma_imagej_types = {"DefaultTubeness": "double"}
        ...
        _calibration_wipp_types = {"DefaultTubeness": "array"}
        _calibration_imagej_types = {"DefaultTubeness": "double[]"}
        if _calibration_wipp_types.get(op, None) != "collection":
            method_call_types.update(
                {
                    method: dtype
                    for method, dtype in _calibration_imagej_types.items()
                    if dtype in supported_data_types
                }
            )
        ...
        _calibration = self.generate_data(
            "calibration",
            _calibration_wipp_types.get(op, None),
