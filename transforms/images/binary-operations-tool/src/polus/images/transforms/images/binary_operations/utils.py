...
# OpenCV structuring element (from cv2.getStructuringElement)
StructuringElement: TypeAlias = np.ndarray

# Threshold for binary invert
_INVERT_THRESHOLD = 0.5


def invert(image: np.ndarray, **_kwargs: object) -> np.ndarray:
    """Invert an image."""
    return (~(image > _INVERT_THRESHOLD)).astype(np.uint8)


def dilate(image: np.ndarray, kernel: StructuringElement, n: int = 1) -> np.ndarray:
...
def erode(image: np.ndarray, kernel: StructuringElement, n: int = 1) -> np.ndarray:
