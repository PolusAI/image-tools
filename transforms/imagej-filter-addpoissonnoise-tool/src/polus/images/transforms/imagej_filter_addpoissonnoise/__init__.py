"""Provides the ImageJ filter addpoissonnoise tool."""
...
logger = logging.getLogger("polus.images.transforms.imagej_filter_addpoissonnoise")
logger.setLevel(POLUS_LOG)


def filter_addpoissonnoise(
    ...
    logger.info(f"{ij.op().help('filter.addPoissonNoise')}")
        ...
        with bfio.BioWriter(out_path, metadata=br_img.metadata) as bw:
            for y_min in range(0, br_img.Y, TILE_SIZE):
                ...
                for x_min in range(0, br_img.X, TILE_SIZE):
                    ...
                    img_tile = ij.op().filter().addPoissonNoise(out_tile, img_tile)
                    out_tile = ij_type.cast_ij_to_image(ij, out_tile)

                    # Fill nan values with 0
                    out_tile[numpy.isnan(out_tile)] = 0
                    bw[y_min:y_max, x_min:x_max, ...] = out_tile
