"""Provides the ImageJ image invert tool."""
...
logger = logging.getLogger("polus.images.transforms.imagej_image_invert")
...
def image_invert(
    ...
    logger.info(f"{ij.op().help('image.invert')}")
        ...
        with bfio.BioWriter(out_path, metadata=br_img.metadata) as bw:
            ...
            for y_min in range(0, br_img.Y, TILE_SIZE):
                ...
                for x_min in range(0, br_img.X, TILE_SIZE):
                    ...
                    img_tile = ij.op().image().invert(out_tile, img_tile)
