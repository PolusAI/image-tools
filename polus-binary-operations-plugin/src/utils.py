...
def fill_holes_binary(image  : np.ndarray, 
    ...
    contour, _ = cv2.findContours(image,mode=cv2.RETR_CCOMP,method=cv2.CHAIN_APPROX_SIMPLE)
        ...
        with BioReader(input_path, backend='java') as br:
            with BioWriter(output_path, backend='java',
                ...
                for window_slice, step_slice in iterate_tiles(shape       = bfio_shape, 
                    ...
                    assert np.all(tile_readlabels >= 0), \
                        f"There are negative numbers in the input tile: {tile_readlabels[tile_readlabels < 0]}"
                    tile_readlabels = tile_readlabels[tile_readlabels > 0] #not just [1:], because there might not be a background
                    
                    if len(tile_readlabels) > 1:
                        assert operation != "inversion", \
                    ...
                    for label in tile_readlabels: # do not want to include zero (the background)
                    ...
                    logger.debug(f"Input Tile has {len(tile_writelabels)} labels & " + \
