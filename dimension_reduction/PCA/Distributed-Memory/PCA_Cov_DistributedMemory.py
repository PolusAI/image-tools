...
def readInput(rank, size):
    ...
    splitFileName = f"{fileName}_{rank:02d}.csv"
    if rank == 0:
...
def ComputePCA(xNormalized):
    ...
    coefficient = 1 / (n - 1)
    covMatrix = torch.matmul(torch.transpose(xNormalized, 0, 1), xNormalized).to(
        args.deviceName,
    )
...
def init_processes(rank, size, backend="mpi"):
    ...
    Duration = datetime.now() - startTime
    logging.info(
        "Rank # " + str(rank) + " says, it took " + str(Duration) + " for COMPUTING PCA",
    )
    print(
        "Rank # " + str(rank) + " says, it took " + str(Duration) + " for COMPUTING PCA",
    )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "outputPath", type=str, nargs="?", default="./PCA_Projected_Data_Final.csv",
    )
