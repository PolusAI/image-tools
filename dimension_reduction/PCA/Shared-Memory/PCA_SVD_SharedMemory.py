...
argparser.add_argument(
    ...
    type=str,
    nargs="?",
    default="./PCA_Projected_Data_Final.csv",
)
...
np.savetxt(
    SingularValuesOutputPath,
    s.cpu().numpy(),
    delimiter=",",
    header="Singluar Values",
    comments="",
)
...
if args.computeStdev == "true":
    ...
    headerLiteral = (
        "Standard Deviation of Data Along each PC, Normalized Value in Percent"
    )
    np.savetxt(
        StdevOutputPath,
        torch.stack((Stdev, normalizedStdev), 1).cpu().numpy(),
        delimiter=",",
...
if args.applySignFlip == "true":
    ...
    temp = (
        torch.eq(absProjectedData, torch.max(absProjectedData, -2, keepdim=True).values)
        .type(torch.FloatTensor)
        .to(device)
    )
    signMatrix = torch.sign(
        torch.sum(projectedData * temp, -2, keepdim=True).to(device),
    ).to(device)
