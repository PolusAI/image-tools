...
class TestScalableFeaturization(unittest.TestCase):
    def test_scalable_features(self):
        ...
        verts, faces, _, _ = measure.marching_cubes(
            (ref_img == 255).astype(np.uint8), 0, allow_degenerate=False,
        )
        ref_features = mesh.mesh_spectral_features(
            verts, faces, k=50, scale_invariant=False,
        )

        assert len(ref_features) == 50
        ...
        assert np.allclose(ref_features, features.flatten(), atol=0.001)
