...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="main", description="Spectral feature generation for segmented objects.",
    )
    ...
    parser.add_argument(
        ...
        dest="numFeatures",
        type=int,
        help="Number of spectral features to calculate.",
        required=True,
    )
    parser.add_argument(
        "--scaleInvariant",
        dest="scaleInvariant",
        type=str,
        help="Calculate scale invariant features.",
        default="False",
        required=False,
    )
    parser.add_argument(
        ...
        dest="limitMeshSize",
        type=int,
        help="Maximum number of mesh faces",
        default=None,
        required=False,
    )
    # Output arguments
    ...
    logger.info(f"scaleInvariant = {scaleInvariant}")
    ...
    inpDir_files = [
        ...
        if f.is_file() and "".join(f.suffixes[-2:]) == ".ome.tif"
    ]
    ...
    for i, f in enumerate(inpDir_files):
        ...
        labels, features = mesh.mesh_and_featurize_image(
            br,
            num_features=numFeatures,
            scale_invariant=scaleInvariant,
            limit_mesh_size=limitMeshSize,
        )
