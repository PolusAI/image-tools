...
def mesh_spectral_features(
    vertices: np.ndarray, faces: np.ndarray, k: int = 50, scale_invariant: bool = False,
) -> np.ndarray:
    ...
    """
    i = np.concatenate(
        [faces[:, 0], faces[:, 0], faces[:, 1], faces[:, 1], faces[:, 2], faces[:, 2]],
    )
    j = np.concatenate(
    ...
    while enough_eigvals is not True and max_iter > 0:
        try:
            ...
            eigvals = scipy.sparse.linalg.eigsh(
                L, sigma=0, which="LM", k=k + 10, return_eigenvectors=False,
            )
        except RuntimeError:
            ...
            n_singular += 1
            logger.warning(
                "Graph laplacian likely singular. Perturbing matrix, but this may introduce inaccuracies.",
            )
            max_iter -= 1
            continue
    ...
    if max_iter <= 0:
        logger.error(
            "Could not solve for the desired number of eigenvalues. Please check the number of connected components in your graph.",
        )
...
def mesh_and_featurize_image(
    ...
    limit_mesh_size: Optional[int] = None,
            ...
            for z in range(0, image.Z, chunk_size[2]):
                ...
                for label in labels:
                    ...
                    if label not in min_bounds:
                        ...
                        min_bounds[label] = np.min(
                            np.stack([min_bounds[label], curr_min]), axis=0,
                        )

                    if label not in max_bounds:
                        max_bounds[label] = curr_max
                    else:
                        max_bounds[label] = np.max(
                            np.stack([max_bounds[label], curr_max]), axis=0,
                        )
    ...
    for i, (label, min_bounds, max_bounds) in enumerate(
        zip(labels, min_bounds.values(), max_bounds.values()),
    ):
        subvol = image[
        ...
        verts, faces, _, _ = measure.marching_cubes(
            (subvol == label).astype(np.uint8), 0, allow_degenerate=False,
        )

        if limit_mesh_size is not None and faces.shape[0] > limit_mesh_size:
            mesh_obj = trimesh.Trimesh(verts, faces).simplify_quadratic_decimation(
                limit_mesh_size,
            )
        ...
        logger.info(
            f"Featurizing ROI {label} ({i + 1}/{len(labels)}) with {verts.shape[0]} vertices.",
        )

        feats = mesh_spectral_features(
            verts, faces, k=num_features, scale_invariant=scale_invariant,
        )
        features[i] = feats
