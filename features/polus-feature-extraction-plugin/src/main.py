...
def read(img_file):
    ...
    img_unit = br.ps_y[1]
    logger.info(f"Reading file\t{img_file.parent}/ {img_file.name}")
...
def box_border_search(label_image, boxsize=3):
    ...
    neighborvals = (
        int_image_flat_transpose[upper_left_int]
    ...
    del (
        ...
        upper_right_int,
        upper_left_int,
        int_image_flat_transpose,
        int_image_flat,
        int_image_int,
    )

    # Divide the pixel averages by the pixel value
    reshape_vals = np.reshape(
        neighborvals, (height - 2 * floor_offset, width - 2 * floor_offset),
    )
    double_image = label_image[
        ceil_offset - 1 : -floor_offset, ceil_offset - 1 : -floor_offset,
    ]
    pix_mask = reshape_vals / double_image
    pad = np.pad(
        pix_mask,
        ((floor_offset, floor_offset), (floor_offset, floor_offset)),
        mode="constant",
    )
    ...
    del (
        ...
        thresh,
        perimeter_indices,
        perimeter_indices_array,
        perimeter_zeros,
        perimeter_int,
        image_flat,
        perimeter_indices_reshape,
        perimeter_flat,
        perimeter_reshape,
    )
...
def neighbors_find(lbl_img, im_label, pixeldistance):
    ...
    for contour in contours:
        ...
        cand = [
            (l, k)
            for pypd, pypd_rn, pxpd, pxpd_rn in zip(pypdd, pypd_rnn, pxpdd, pxpd_rnn)
        ...
        values = [
            lbl_img[l][k]
            for l, k in cand
            if (l < shape_img[0] and k < shape_img[1] and l >= 0 and k >= 0)
        ]
        nei.append(values)
...
def feret_diameter(lbl_img, boxsize, thetastart, thetastop):
    ...
    for counts in objbounds_counts:
        ...
        uniqueindices_concate = np.concatenate(
            (uniqueindices_x_reshape, uniqueindices_y_reshape), axis=1,
        )
        uniqueindices_list.append(uniqueindices_concate)
        del (
            uniqueindices_concate,
            uniqueindices_x,
            uniqueindices_y,
            uniqueindices_x_reshape,
            uniqueindices_y_reshape,
        )
    ...
    if neighbors > 2:
            ...
            (
                ...
                - (
                    ...
                    / (
                        math.sqrt(
                            (4 * area)
                            / (neighbors * (1 / (math.tan(math.pi / neighbors)))),
                        )
                    )
                )
            )
        ...
        for ib in range(0, len(area_uniq)):
            for ic in range(ib + 1, len(area_uniq)):
                area_ratio = 1 - math.sqrt(
                    (1 - (area_uniq[ib] / area_uniq[ic]))
                    * (1 - (area_uniq[ib] / area_uniq[ic])),
                )
        ...
        for ib in range(0, len(perim_uniq)):
            for ic in range(ib + 1, len(perim_uniq)):
                perim_ratio = 1 - math.sqrt(
                    (1 - (perim_uniq[ib] / perim_uniq[ic]))
                    * (1 - (perim_uniq[ib] / perim_uniq[ic])),
                )
...
def feature_extraction(
    ...
    def orientation(seg_img, *args):
        ...
        def compute_M(data):
            ...
            """"""
            return csr_matrix(
                (cols, (data_sc.ravel(), cols)), shape=(len(label) + 1, data_sc.size),
            )
    ...
    def bbox_ymin(*args):
        ...
        logger.debug(
            "Completed extracting boundingbox_ymin for " + seg_file_names1.name,
        )
        return data_dict
    ...
    def bbox_xmin(*args):
        ...
        logger.debug(
            "Completed extracting boundingbox_xmin for " + seg_file_names1.name,
        )
        return data_dict
    ...
    def bbox_width(*args):
        ...
        logger.debug(
            "Completed extracting boundingbox_width for " + seg_file_names1.name,
        )
        return data_dict
    ...
    def bbox_height(*args):
        ...
        logger.debug(
            "Completed extracting boundingbox_height for " + seg_file_names1.name,
        )
        return data_dict
    ...
    def equivalent_diameter(seg_img, units, *args):
        ...
        if unitLength and not embeddedpixelsize:
            ...
            data_dict = data_dict1
        logger.debug(
            "Completed extracting equivalent diameter for " + seg_file_names1.name,
        )
        return data_dict
    ...
    def major_axis_length(seg_img, units, *args):
        ...
        if unitLength and not embeddedpixelsize:
            ...
            data_dict = data_dict1
        logger.debug(
            "Completed extracting major axis length for " + seg_file_names1.name,
        )
        return data_dict
    ...
    def minor_axis_length(seg_img, units, *args):
        ...
        if unitLength and not embeddedpixelsize:
            ...
            data_dict = data_dict1
        logger.debug(
            "Completed extracting minor axis length for " + seg_file_names1.name,
        )
        return data_dict
    ...
    def mean_intensity(*args):
        ...
        if label_image is not None:
            ...
            imgs = [region.image for region in regions]
            data_dict = [
                (np.mean(intensity[seg]))
                for intensity, seg in zip(intensity_images, imgs)
            ]
        else:
    ...
    def skewness(*args):
        ...
        if label_image is not None:
            ...
            imgs = [region.image for region in regions]
            data_dict = [
                skew(intensity[seg], axis=0, bias=True)
                for intensity, seg in zip(intensity_images, imgs)
            ]
        else:
    ...
    def kurtosis(*args):
        ...
        if label_image is not None:
            ...
            imgs = [region.image for region in regions]
            data_dict = [
                kurto(intensity[seg], axis=0, fisher=False, bias=True)
                for intensity, seg in zip(intensity_images, imgs)
            ]
        else:
            data_dict = kurto(
                intensity_image.reshape(-1), axis=0, fisher=False, bias=True,
            )
    ...
    def neighbors(seg_img, *args):
        ...
        results = executor.map(
            neighbors_find, repeat(seg_img), label, repeat(pixelDistance),
        )
    ...
    def poly_hex_score(seg_img, units):
        ...
        return [
            ...
            for area_metric, perimeter_metric, neighbor_metric, solidity_metric, maxferet_metric, minferet_metric in zip(
                poly_area,
                poly_peri,
                poly_neighbor,
                poly_solidity,
    ...
    def polygonality_score(seg_img, units, *args):
        ...
        logger.debug(
            "Completed extracting polygonality score for " + seg_file_names1.name,
        )
    ...
    def hexagonality_score(seg_img, units, *args):
        ...
        logger.debug(
            "Completed extracting hexagonality score for " + seg_file_names1.name,
        )
    ...
    def hexagonality_sd(seg_img, units, *args):
        ...
        logger.debug(
            "Completed extracting hexagonality standard deviation for "
            + seg_file_names1.name,
        )
    ...
    def all(seg_img, units, int_img):
        ...
        if unitLength and not embeddedpixelsize:
            [dt_pixel / pixelsPerunit for dt_pixel in all_maxferet]
            [dt_pixel / pixelsPerunit for dt_pixel in all_minferet]
        else:
            pass
        ...
        return (
            ...
            all_bboxwidth,
            all_bboxheight,
            all_major_axis_length,
            all_minor_axis_length,
            all_eccentricity,
            all_orientation,
            all_convex,
            all_euler_number,
            all_equivalent_diameter,
            all_solidity,
            all_peri,
            all_maxferet,
            all_minferet,
            all_neighbor,
            all_polygonality_score,
            all_hexagonality_score,
            all_hexagonality_sd,
            all_kurtosis,
            all_max_intensity,
            all_mean_intensity,
    ...
    if not features:
        msg = "Select features for extraction."
    ...
    if unitLength and not embeddedpixelsize and not pixelsPerunit:
    ...
    if label_image is not None:
        ...
        label_nt_touching[label_nt_touching >= len(border_cells)] = (
            len(border_cells) - 1
        )  # Limit it
        border_cells[label_nt_touching] = False
        if intensity_image is None:
            # Create column label and image
            data = {"mask_image": title, "label": label}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="main",
        description="Everything you need to start a Feature Extraction plugin.",
    )
    parser.add_argument(
        "--features",
        dest="features",
    ...
    parser.add_argument(
        ...
        dest="filePattern",
        type=str,
        help="The filepattern used to match files with each other.",
        required=True,
    )
    parser.add_argument(
        "--csvfile",
        dest="csvfile",
        type=str,
        help="Save csv as separate or single file",
        required=True,
    )
    parser.add_argument(
        "--embeddedpixelsize",
        dest="embeddedpixelsize",
        type=str,
        help="Embedded pixel size if present",
        required=False,
    )
    parser.add_argument(
        "--pixelsPerunit",
    ...
    parser.add_argument(
        ...
        dest="unitLength",
        type=str,
        help="Required units for features extracted",
        required=False,
    )
    parser.add_argument(
        "--intDir",
        dest="intDir",
        type=str,
        help="Intensity image collection",
        required=False,
    )
    parser.add_argument(
        "--pixelDistance",
        dest="pixelDistance",
        type=int,
        help="Pixel distance to calculate the neighbors touching cells",
        required=False,
    )
    parser.add_argument(
        "--segDir",
        dest="segDir",
        type=str,
        help="Segmented image collection",
        required=False,
    )
    parser.add_argument(
    ...
    if intDir and not segDir:
        if "all" in features:
            msg = "No labeled/segmented image specified."
            raise ValueError(msg)
        ...
        if "all" in features:
            msg = "No intensity image specified."
            raise ValueError(msg)
            features = [i for i in features if i in intensity_features]
    ...
    if segDir and intDir:
        if len(files_seg) == 0 and len(files_int) == 0:
            msg = "Could not find files matching filepattern"
            raise ValueError(msg)
    elif segDir and not intDir:
        if len(files_seg) == 0:
            msg = "Could not find labeled/segmented image files matching filepattern"
            raise ValueError(
                msg,
            )
    elif intDir and not segDir and len(files_int) == 0:
        raise ValueError(
            "Could not find intensity image files matching filepattern",
        )
    ...
    if not segDir:
        for intfile in files_int:
            ...
            if csvfile == "separatecsv":
                ...
                df.to_csv(
                    r"%s.csv" % title, index=None, header=True, encoding="utf-8-sig",
                )
            else:
        ...
        for img_file in itertools.zip_longest(files_seg, files_int):
            ...
            if intDir:
                # Get matching files
                files = configfiles_int.get_matching(
                    **{
                        k.upper(): v
                        for k, v in img_file[0][0].items()
                        if k not in ["file", "c"]
                    },
                )
                if files is not None:
                    if len(files) == 0 and (
                        all(fe not in intensity_features for fe in features)
                    ):
                        intensity_image = None

                    elif len(files) == 0 and (
                        any(fe in intensity_features for fe in features)
                    ):
                        logger.warning(
                            f"Could not find intensity files matching label image, {img_file[0][0]['file'].name}. Skipping...",
                        )
                        if df is None:
                            continue
                    else:
                    ...
                    if len(files_seg) != len(files_int):
                        msg = "Number of labeled/segmented images is not equal to number of intensity images"
                        raise ValueError(
                            msg,
                        )
            ...
            if not intDir or files == [] or files is None:
                channel = None
                int_filename = None
                if intDir and files is None:
            ...
            if csvfile == "singlecsv" and (files == "" or files is None or files == []):
                df_csv = df_csv.append(df)
            elif csvfile == "separatecsv":
                if df.empty:
                    msg = "No output to save as csv files"
                    raise ValueError(msg)
                else:
                    logger.info(
                        "Saving dataframe to csv for " + img_file[0][0]["file"].name,
                    )
    ...
    if csvfile == "singlecsv":
        if df_csv.empty:
            ...
            logger.info(
                f"Saving dataframe to csv file for all images in {outDir}",
            )
            ...
            df_csv.to_csv(
                r"Feature_Extraction.csv", index=None, header=True, encoding="utf-8-sig",
            )
