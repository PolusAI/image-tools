...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="model",
        type=str,
        help="Pre-trained ImageNet model to use for featurization",
        required=True,
    )
    parser.add_argument(
        "--resolution",
        dest="resolution",
        type=str,
        help="Resolution to which the input images are scaled",
        required=False,
    )
    # Output arguments
    parser.add_argument(
    ...
    if not os.path.exists(output_dir):
        logger.info(
            f"Output directory {output_dir} does not exist. Creating directory.",
        )
    ...
    if resolution is not None:
        ...
        if match is None:
            logger.error(
                f"Resolution should be in the following format: 500x500. You entered: {resolution}.",
            )
            sys.exit()
    ...
    if model not in valid_models:
        logger.error(
            f'You requested model {model}. Model must be one of the following: {", ".join(valid_models)}.',
        )
        sys.exit()
    ...
    if not isinstance(imagenet_model, tf.keras.Model):
        logger.error("Unable to load requested model!")
        sys.exit()
