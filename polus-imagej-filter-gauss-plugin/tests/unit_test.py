...
class UnitTest(unittest.TestCase):
    ...
    def test_plugin(self):
        ...
        projectName = 'ImageJ filter gauss'
        ...
        _opName = op
        _in1_wipp_types = {'GaussRAISingleSigma': 'collection', 'DefaultGaussRAI': 'collection'}
        _in1_imagej_types =  {'GaussRAISingleSigma': 'RandomAccessibleInterval', 'DefaultGaussRAI': 'RandomAccessibleInterval'}
        ...
        _sigma_wipp_types = {'GaussRAISingleSigma': 'number'}
        _sigma_imagej_types =  {'GaussRAISingleSigma': 'double'}
        if _sigma_wipp_types.get(op, None) != 'collection':
            method_call_types.update({method:dtype for method,dtype in _sigma_imagej_types.items() \
                if dtype in supported_data_types})
        _sigmas_wipp_types = {'DefaultGaussRAI': 'array'}
        _sigmas_imagej_types =  {'DefaultGaussRAI': 'double[]'}
        if _sigmas_wipp_types.get(op, None) != 'collection':
            method_call_types.update({method:dtype for method,dtype in _sigmas_imagej_types.items() \
                if dtype in supported_data_types})
        ...
        _sigma = self.generate_data(
            'sigma',
            _sigma_wipp_types.get(op, None), 
            method_call_types.get(op, None)
            )
        _sigmas = self.generate_data(
            'sigmas',
            _sigmas_wipp_types.get(op, None), 
            method_call_types.get(op, None)
            )
