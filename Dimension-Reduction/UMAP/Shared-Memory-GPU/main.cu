...
int main(int argc, char ** argv) {
	...
	for (int i = 0; i < edgeCounts; ++i) {
		pointer_epochs_per_sample[i]=epochs_per_sample[i];
		pointer_head[i]=head[i];
		pointer_tail[i]=tail[i];
	}
	 ...
	 */
	int requiredBlocks= edgeCounts/TPB+1;
	/**
	 *  Launch a kernel to initialize some arrays on GPU memory
	 */
	initializeEpochs<<<requiredBlocks,TPB>>> (edgeCounts, device_epochs_per_sample,device_epoch_of_next_sample,device_epochs_per_negative_sample, device_epoch_of_next_negative_sample);
	gpuErrchk(cudaPeekAtLastError());
	...
	for (int n = 1; n <= n_epochs; ++n) { 
		...
		SGDEngine<<< requiredBlocks ,TPB>>>(device_embedding1D, device_head, device_tail, alpha, N, DimLowSpace, aValue, bValue, edgeCounts, device_epochs_per_sample, device_epoch_of_next_sample, device_epochs_per_negative_sample, device_epoch_of_next_negative_sample ,n, move_other, device_state);
	 ...
	 */
	cudaFree (device_embedding1D);
	cudaFree (device_epochs_per_sample);
	cudaFree (device_epoch_of_next_sample);
	cudaFree (device_epochs_per_negative_sample);
	cudaFree (device_epoch_of_next_negative_sample);
	 ...
	 */
	delete [] embedding1D;
	delete [] pointer_epochs_per_sample;
	delete [] pointer_head;
	delete [] pointer_tail;
	delete [] embedding;
	/**
