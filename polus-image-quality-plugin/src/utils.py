...
class Image_quality:
    ...
    #     pixel_count = np.product(image_data.shape)
    ...
    #         pixel_max = 0
    #     else:
    #         pixel_sum = np.sum(image_data)
    #         pixel_mean = pixel_sum / float(pixel_count)
    #         pixel_std = np.std(image_data)
    #         pixel_median = np.median(image_data)
    #         pixel_mad = np.median(np.abs(image_data - pixel_median))
    #         pixel_min = np.min(image_data)
    #         pixel_max = np.max(image_data)


    #     return pixel_sum, pixel_mean, pixel_std, pixel_median, pixel_mad, pixel_min, pixel_max
...
def write_csv(*args):
    ...
    file_exists = os.path.isfile(args[2])
...
# #     pattern = re.compile("r(?P<row>\d+)c(?P<column>\d+)f(?P<site>\d+)p_(?P<zplane>\d+)-(?P<Number>\d+)_-ch(?P<Channel>\d+)")
...
# #     zplane= match.group("zplane")

# #     channel= match.group("Channel")

# #     ## Extracting Well information from Rows and Columns
...
# #     # Comibining row and column values 

# #     rc = row+col

# #     well = well_assignment.get(rc[0:2])+rc[2:]

# #     return str(well), str(site), str(zplane), str(channel)
