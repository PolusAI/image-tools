...
def main(_opName: str,
    ...
    # Validate in1
    in1_types = { 
        "DefaultIntegralImg": "RandomAccessibleInterval",
        "SquareIntegralImg": "RandomAccessibleInterval",
        "WrappedIntegralImg": "RandomAccessibleInterval",
    }
    
    # Check that all inputs are specified 
    if _in1 == None and _opName in list(in1_types.keys()):
    ...
    try:
        for ind, (in1_path,) in enumerate(zip(*args)):
            if in1_path != None:
                ...
                in1 = ij_converter.to_java(np.squeeze(in1_br[:,:,0:1,0,0]),in1_type)
                metadata = in1_br.metadata
                fname = in1_path.name
                dtype = ij.py.dtype(in1)
            logger.info('Running op...')
            if _opName == "DefaultIntegralImg":
                out = ij.op().image().integral(in1)
            elif _opName == "SquareIntegralImg":
                out = ij.op().image().integral(in1)
            elif _opName == "WrappedIntegralImg":
                out = ij.op().image().integral(in1)
            ...
            logger.info('Saving...')
            out_array = ij_converter.from_java(out,out_types[_opName])
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(prog='main', description='DefaultIntegralImg,SquareIntegralImg,WrappedIntegralImg')
    
    # Add command-line argument for each of the input arguments
    parser.add_argument('--opName', dest='opName', type=str,
                        help='Operation to perform', required=False)
