...
if __name__=="__main__":
    ...
    parser.add_argument('--stackofinterest', dest='stack_ofinterest', type=str,
                        help="Dataframe for all the stitched vectors", required=True)
    ...
    stackofinterest = args.stack_ofinterest

    channels = stackofinterest.split()
    channels = [Path(input_dir + z) for z in channels]
    image = channels[0]
    # for item in channels:
    #     zval = filepattern.parse_filename(item, pattern='r01_x{xx}_y{yy}_z{zz}.ome.tif',variables='z')
    #     channelvals.append(zval['z'])
    # print(channelvals)


    # Initialize the logger    
    logging.basicConfig(format='%(asctime)s - %(name)s - {} - %(levelname)s - %(message)s'.format(stackcount),
                        datefmt='%d-%b-%y %H:%M:%S')
    ...
    # br = BioReader('dA30_5_dA30.Labels.ome.tif')
