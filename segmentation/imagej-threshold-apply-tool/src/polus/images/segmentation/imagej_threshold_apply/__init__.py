...
logger = logging.getLogger("polus.images.segmentation.imagej_threshold_apply")
...
def threshold_apply(
    ...
    with bfio.BioReader(inp_path) as br:
        ...
        threshold = ij_type.cast_primitive(threshold)
        ...
        with bfio.BioWriter(out_path, metadata=br.metadata) as bw:
            for y_min in range(0, br.Y, TILE_SIZE):
                ...
                for x_min in range(0, br.X, TILE_SIZE):
                    ...
                    img = ij.op().threshold().apply(img, threshold)
