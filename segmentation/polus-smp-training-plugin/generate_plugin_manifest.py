...
INPUTS = {
    ...
    "encoderWeights": {
        ...
        "type": "enum",
        "options": {"values": sorted(utils.ENCODER_WEIGHTS)},
    },
    "optimizerName": {
        "description": (
...
def bump_version(debug: bool) -> str:
    ...
    if debug:
        if "debug" in version:
            ...
            version = f"{version}debug{1 + int(debug)!s}"
        else:
...
def create_ui():
    ...
    for key, values in INPUTS.items():
        ...
        if key not in COMMON_ARGS and key != "inferenceMode":
...
def generate_manifest(debug: bool):
    ...
    manifest = {
        ...
        "author": "Gauhar Bains (gauhar.bains@labshare.org), Najib Ishaq (najib.ishaq@axleinfo.com), Madhuri Vihani (madhuri.vihani@nih.gov), Benjamin Houghton (benjamin.houghton@axleinfo.com)",
        "institution": "National Center for Advancing Translational Sciences, National Institutes of Health",
        "repository": "https://github.com/PolusAI/polus-plugins/tree/dev/segmentation",
        "website": "https://ncats.nih.gov/preclinical/core/informatics",
        "citation": "",
        "containerId": f"polusai/smp-training-plugin:{version}",
