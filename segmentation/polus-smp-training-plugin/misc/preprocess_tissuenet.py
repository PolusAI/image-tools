...
if not os.path.isdir(OUTPUT_DIR):
    msg = f"Output Directory ({OUTPUT_DIR}) does not exist; please create manually"
    raise ValueError(
        msg,
    )
...
def convert_to_boundarymaps(groundtruth_boundary_array):
    ...
    gt_cntrbnry_2pxlerro_arr[
        label_binary[:, :, 1] == 1
    ] = 1  # center values are set to 1
    ...
    groundtruth_multiclass_array[
        dilated_label_binary[:, :, 2] == 1
    ] = 1  # border values are set to 1
    groundtruth_multiclass_array[
        dilated_label_binary[:, :, 1] == 1
    ] = 2  # center values are set to 2
    ...
    return (
        groundtruth_multiclass_array,
        groundtruth_borderbinary_array,
        gt_cntrbnry_3pxlerro_arr,
        gt_cntrbnry_2pxlerro_arr,
    )
...
def save_file(npz_location, classofdata, typeofdata):
    ...
    groundtruth_multiclass_directory = os.path.join(
        output_directory, "groundtruth_multiclass",
    )
    if not os.path.isdir(groundtruth_multiclass_directory):
        os.mkdir(groundtruth_multiclass_directory)
    groundtruth_borderbinary_directory = os.path.join(
        output_directory, "groundtruth_borderbinary",
    )
    if not os.path.isdir(groundtruth_borderbinary_directory):
        os.mkdir(groundtruth_borderbinary_directory)
    groundtruth_centerbinary3_directory = os.path.join(
        output_directory, "groundtruth_centerbinary_2pixelsmaller",
    )
    if not os.path.isdir(groundtruth_centerbinary3_directory):
        os.mkdir(groundtruth_centerbinary3_directory)
    groundtruth_centerbinary2_directory = os.path.join(
        output_directory, "groundtruth_centerbinary_1pixelsmaller",
    )
    ...
    for ex in range(num_examples):
        ...
        (
            ...
            gt_cntrbnry_3pxlerro_arr,
            gt_cntrbnry_2pxlerro_arr,
        ) = convert_to_boundarymaps(groundtruth_multiclass_array)

        # all output (images and groundtruths will have the same name, just saved in different directories)
        file_outputname = f"{classofdata}_{typeofdata}_{ex}.tif"
        ...
        groundtruth_multiclass_file = os.path.join(
            groundtruth_multiclass_directory, file_outputname,
        )
        groundtruth_borderbinary_file = os.path.join(
            groundtruth_borderbinary_directory, file_outputname,
        )
        groundtruth_centerbinary3_file = os.path.join(
            groundtruth_centerbinary3_directory, file_outputname,
        )
        groundtruth_centerbinary2_file = os.path.join(
            groundtruth_centerbinary2_directory, file_outputname,
        )
        ...
        Image.fromarray(groundtruth_borderbinary_array).save(
            groundtruth_borderbinary_file,
        )
        ...
        if ex == 0:  # checking the first image qualitatively
            ...
            axes[0, 1].set_title(
                f"Ground Truth: {np.min(groundtruth_array)}-{np.max(groundtruth_array)}",
            )
            axes[1, 1].set_title(
                f"Ground Truth Binary Centers 1 pixel errosion: {np.min(gt_cntrbnry_2pxlerro_arr)}-{np.max(gt_cntrbnry_2pxlerro_arr)}",
            )
            axes[1, 0].set_title(
                f"Ground Truth Multiclass: {np.min(groundtruth_multiclass_array)}-{np.max(groundtruth_multiclass_array)}",
            )
            axes[2, 0].set_title(
                f"Ground Truth Binary Borders: {np.min(groundtruth_borderbinary_array)}-{np.max(groundtruth_borderbinary_array)}",
            )
            axes[2, 1].set_title(
                f"Ground Truth Binary Centers 2 pixel errosion: {np.min(gt_cntrbnry_3pxlerro_arr)}-{np.max(gt_cntrbnry_3pxlerro_arr)}",
            )

            fig.suptitle(
                f"Example Plot for {classofdata}'s Data ({typeofdata})",
            )
            plot_name = f"{classofdata}_{typeofdata}.jpg"
            plt.savefig(os.path.join(OUTPUT_DIR, plot_name))

        print(
            "Saved {}'s {} data ({}/{})".format(
                classofdata, typeofdata, ex, num_examples - 1,
            ),
        )
    print(f"Saved all of {classofdata}'s {typeofdata} data")
    print(" ")
