...
class DatasetforPytorch(Dataset):
    def __init__(self, images_dir, masks_dir, preprocessing=None, augmentations=None) -> None:
        self.images_fps = [
            os.path.join(images_dir, image) for image in os.listdir(images_dir)
        ]
        self.masks_fps = [
            os.path.join(masks_dir, mask) for mask in os.listdir(masks_dir)
        ]
        self.preprocessing = (
            preprocessing  # this is a function that is getting intialized
        )
        self.augmentations = (
            augmentations  # this is a function that is getting initialized
        )
    ...
    def __getitem__(self, i):
        ...
        assert np.isnan(image).any() is False
        assert np.isinf(image).any() is False

        mask = np.reshape(mask, (1, mask.shape[0], mask.shape[1])).astype("float32")
        assert np.isnan(mask).any() is False
        assert np.isinf(mask).any() is False
...
def get_validation_augmentation():
    ...
    test_transform = [albu.PadIfNeeded(256, 256)]
...
def plot_fromnohup(file_name):
    ...
    valid_log_f = []
    nohupfile = (
        "/home/vihanimm/SegmentationModelToolkit/workdir/pytorch_binary/nohup.out"
    )
    with open(nohupfile) as nohup:
        for line in nohup:
            line = line.rstrip()
...
def train(
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=8, shuffle=False, num_workers=12,
    )
    ...
    optimizer = torch.optim.Adam(
        UNET_basicmodel.parameters(), lr=0.0001,
    )  # only calculate for the parameters specified
    ...
    while stop_the_training is False:
        epoch_loss = 0
        epoch_iou = 0
        epoch_fscore = 0
        for data, target in train_loader:
            ...
            assert torch.isnan(output).any() is False
            assert torch.isinf(output).any() is False
            losses = loss.forward(
                probability, target,
            )  # take inputs, and pass thru till we get to the
        ...
        print(
            f"TRAIN EPOCH {epoch}: Loss {epoch_loss}, F Score {epoch_fscore}, Iou Score {epoch_iou}",
        )
        ...
        print(
            f"VALID EPOCH {epoch}: Loss {epoch_loss}, F Score {epoch_fscore}, Iou Score {epoch_iou}",
        )
        ...
        if epoch_fscore > max_score:
            ...
            print(f"MODEL SAVED with F Score of {epoch_fscore}")
            early_stopping_counter = 0
        else:
...
def visualize_output(
    model_pathway, data_directory, device, output_dir, groundtruth_basedir,
):
    def add_to_axis(image, groundtruth, threshold, axis=None):
        ...
        j_score = jaccard_score(
            y_true=groundtruth, y_pred=new_img, average=None, zero_division="warn",
        )
        ...
        if axis is not None:
            axis.imshow(new_img)
            axis.set_title(
                f"Threshold: {threshold} - F1: {f1_score}, JACCARD: {j_score}",
            )
    ...
    # create test dataset
    test_dataset = DatasetforPytorch(
        images_dir=x_test_dir, masks_dir=y_test_dir,
    )
    ...
    for i in tqdm(range(len(nums))):
        ...
        fig, (
            (ax_img, ax_groundtruth, ax_prediction),
            (ax_pred1, ax_pred2, ax_pred3),
            (ax_pred4, ax_pred5, ax_pred6),
            (ax_pred7, ax_pred8, ax_pred9),
        ...
        f1_score_1, j_score_1 = add_to_axis(
            image=pr_mask, groundtruth=gt_mask, threshold=0.1, axis=ax_pred1,
        )
        f1_score_2, j_score_2 = add_to_axis(
            image=pr_mask, groundtruth=gt_mask, threshold=0.2, axis=ax_pred2,
        )
        f1_score_3, j_score_3 = add_to_axis(
            image=pr_mask, groundtruth=gt_mask, threshold=0.3, axis=ax_pred3,
        )
        f1_score_4, j_score_4 = add_to_axis(
            image=pr_mask, groundtruth=gt_mask, threshold=0.4, axis=ax_pred4,
        )
        f1_score_5, j_score_5 = add_to_axis(
            image=pr_mask, groundtruth=gt_mask, threshold=0.5, axis=ax_pred5,
        )
        f1_score_6, j_score_6 = add_to_axis(
            image=pr_mask, groundtruth=gt_mask, threshold=0.6, axis=ax_pred6,
        )
        f1_score_7, j_score_7 = add_to_axis(
            image=pr_mask, groundtruth=gt_mask, threshold=0.7, axis=ax_pred7,
        )
        f1_score_8, j_score_8 = add_to_axis(
            image=pr_mask, groundtruth=gt_mask, threshold=0.8, axis=ax_pred8,
        )
        f1_score_9, j_score_9 = add_to_axis(
            image=pr_mask, groundtruth=gt_mask, threshold=0.9, axis=ax_pred9,
        )
...
def main():
    ...
    ), f"{output_model_directory} is not an existing directory for models"
    assert os.path.isdir(
        data_basedirectory,
    ), f"{data_basedirectory} is not an existing directory for data"
    assert os.path.isdir(
        file_directory,
    ), f"{file_directory} is not existing directory for {__file__}"

    output_model_file = os.path.join(output_model_directory, "UNET_BinaryDiceLoss.pth")
    output_train_json = os.path.join(
        output_model_directory, f"{output_model_file[:-4]}_train.json",
    )
    output_valid_json = os.path.join(
        output_model_directory, f"{output_model_file[:-4]}_valid.json",
    )
    output_score_graph = os.path.join(
        output_model_directory, f"{output_model_file[:-4]}.jpg",
    )
    ...
    plot_histories(
        train_logs=train_history, valid_logs=valid_history, file_name=output_score_graph,
    )

    visualize_output(
        model_pathway=output_model_file,
        data_directory=data_directory,
        device=device,
