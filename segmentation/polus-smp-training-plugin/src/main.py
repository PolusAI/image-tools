...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="main", description="Segmentation models training plugin",
    )
    ...
    assert inference_mode in (
        "active",
        "inactive",
    ), "--inferenceMode must be one of 'active' or 'inactive'."
    ...
    if inference_mode:
        ...
        assert (
            images_inference_dir.exists()
        ), f"Directory does not exist: {images_inference_dir}"
        ...
        assert (
            images_train_dir.exists()
        ), f"Directory does not exist: {images_train_dir}"
        assert (
            labels_train_dir.exists()
        ), f"Directory does not exist: {labels_train_dir}"
        assert (
            images_valid_dir.exists()
        ), f"Directory does not exist: {images_valid_dir}"
        assert (
            labels_valid_dir.exists()
        ), f"Directory does not exist: {labels_valid_dir}"
    ...
    if encoder_base is not None:
        if encoder_base not in utils.ENCODERS:
            ...
            if checkpoint["encoder_variant"] not in available_variants:
                ...
                if (checkpoint["encoder_weights"] is not None) and (
    ...
    if len(error_messages) > 0:
        error_messages = (
            ["Oh no! Something went wrong", *error_messages, "See the README for details."]
        )
        error_message = "\n".join(error_messages)
        logger.error(error_message)
        raise ValueError(error_message)

    logger.info("Using input arguments:")
    ...
    batch_size = (
        max_batch_size if batch_size is None else min(batch_size, max_batch_size)
    )
    ...
    if inference_mode:
        ...
        epoch_iterators = training.initialize_epoch_iterators(
            model=model,
            loss=loss,
            metrics=[metric() for metric in utils.METRICS.values()],
            device=device,
            optimizer=optimizer,
        )
