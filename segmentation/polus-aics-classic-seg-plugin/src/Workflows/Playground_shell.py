...
from aicssegmentation.core.utils import get_3dseed_from_mid_frame
...
def segment_images(inpDir, outDir, config_data):
    ...
    for i, f in enumerate(inpDir_files):
        logger.info(f"Segmenting image : {f}")
        ...
        seed = get_3dseed_from_mid_frame(
            np.logical_xor(bw_fill_mid_z, bw_mid_z), struct_img.shape, mid_z, hole_min,
        )
